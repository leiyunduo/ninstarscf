<template>
  <div class="view_chain">
    <div class="line_detail">
      ...
      <div class="chainNavLine" v-show="selIndex === 0" :style="element ? ' box-shadow: 0 2px 8px rgba(0, 0, 0, 0.15);' : ''">
        <ring :chainId="chainId" :chainType="1" :element="element"/>
      </div>
      <ul class="view_nav" :style="selIndex === 0 ? 'top: 90px;' : ''">
        ...
        <li v-for="(item, index) in navList" :key="index" @click="changeTab($event, index)"><span :class="index === selIndex ? 'sel':''">{{item.name}}</span></li>
      </ul>
      <div v-if="selIndex === 0" class="chain_list" style="margin-top: 120px">
        <div class="bottom">
          <dl>
            ...
            <dt>
              <img class="barIcon" src="static/img/tickets/yinquan.png" alt="">
            </dt>
            <dd>
              <p class="one">银券总额(万)：<span>{{convertData(ticket.bank_ticket.sum) || 0}}</span></p>
          ...
          <dl>
            ...
            <dd>
              <p class="one">商券总额(万)：<span>{{convertData(ticket.business_ticket.sum) || 0}}</span></p>
          ...
          <dl>
            ...
            <dd>
              <p class="one">数券总额(万)：<span>{{convertData(ticket.digital_ticket.sum) || 0}}</span></p>
        ...
        <div class="bottom">
          <dl>
            ...
            <dd>
              <p class="one">红条总额(万)：<span>{{convertData(ticket.red_bar.sum) || 0}}</span></p>
              <p class="two"><span>{{ticket.red_bar.count || 0}}</span> 张</p>
            </dd>
          </dl>
          <dl>
            <dt><img class="barIcon" src="static/img/tickets/huangtiao.png" alt=""></dt>
            <dd>
              <p class="one">黄条总额(万)：<span>{{convertData(ticket.yellow_bar.sum) || 0}}</span></p>
              <p class="two"><span>{{ticket.yellow_bar.count || 0}}</span> 张</p>
            </dd>
          </dl>
          <dl>
            <dt><img class="barIcon" src="static/img/tickets/baitiao.png" alt=""></dt>
            <dd>
              <p class="one">白条总额(万)：<span>{{convertData(ticket.white_bar.sum) || 0}}</span></p>
              <p class="two"><span>{{ticket.white_bar.count || 0}}</span> 张</p>
            </dd>
          </dl>
        </div>
        <div class="chain-box">
              ...
              <div class="chain-container">
                <div class="first_view">
                  <div class="first_chain">
                    <div class="chain_title">
                      ...
                      <div class="tooltips-group">
                        <div class="tooltips-group__item">
                          ...
                          <span class="tips-content">未开始记账</span>
                        </div>
                        <div class="tooltips-group__item">
                    ...
                    <contract ref="refContractRece" :chainType="`1`" :type="`1`" :contractType="`1`" :id='chainId' @changeId='handleChild' @orderWarn="handleOrder"/>
                  </div>
                </div>
                <div class="second_view">
                  <div class="second_chain">
                    ...
                    <order :chainType="`1`" :type="`1`" :orderwarn="orderWarn.receive" :id='contractIds.receive' @changeOrderId="updataOrderId" @changSettlementId="updataSettlementId" @isWarning="hasWarning"/>
                  </div>
                  <div class="second_chain">
                    <div class="chain_title">
                      <div class="symbol_middle_orange"><div></div></div>
                    ...
                    <settlement :chainType="`1`" :type="`1`" :overtime="overTime.receive" :id='contractIds.receive' :orderId="orderIds.receive" :settleId="settleIds.receive"/>
                  </div>
                  <div class="second_chain">
                    <div class="chain_title">
                      <div class="symbol_middle_orange"><div></div></div>
              ...
              <div class="chain-container border_hide">
                <div class="first_view border_show">
                  <div class="first_chain">
                    ...
                    <contract ref='refContractPay' :chainType="`1`" :type="`2`" :id='chainId' :contractType="`2`"  @changeId='handleChild' @orderWarn="handleOrder" />
                  </div>
                </div>
                <div class="second_view">
                  <div class="second_chain">
                    ...
                    <order :chainType="`1`" :type="`2`" :orderwarn="orderWarn.payment" :id='contractIds.payment' @changeOrderId="updataOrderId" @changSettlementId="updataSettlementId" @isWarning="hasWarning"/>
                  </div>
                  <div class="second_chain">
                    <div class="chain_title">
                      <div class="symbol_middle_blue"><div></div></div>
                    ...
                    <settlement :chainType="`1`" :type="`2`" :overtime="overTime.payment" :id='contractIds.payment' :orderId="orderIds.payment" :settleId="settleIds.payment"/>
                  </div>
                  <div class="second_chain">
                    <div class="chain_title">
                      <div class="symbol_middle_blue"><div></div></div>
      ...
      <div v-if="selIndex === 1" class="chain_list">
        <role-register :chainId="chainId"/>
      </div>
      <div v-if="selIndex === 2" class="chain_list">
        <button class="data_time_model" :class="dataTime === index ? 'active' : ''" v-for="(item, index) in dataTimeModel" :key="index" @click="dataModel(index)">{{item}}</button>
        <data-analyse :isChain="true" :chainId="chainId" v-if='dataTime === 0' :bid='bid' />
...
import Contract from '@/components/chain-view/base/chain_modules/chain_contract'
import Order from '@/components/chain-view/base/chain_modules/chain_order'
import Settlement from '@/components/chain-view/base/chain_modules/chain_settlement'
import SettlementContract from '@/components/chain-view/base/chain_modules/chain_settlement_contract'
import RoleRegister from '../chain_roles/ReceiveRoleRegister'
// 大数据资产与分析
import timeModel from '../chain_roles/timeModel'
import dataAnalyse from '@/components/public/assets'
import Attachments from '../chain_attachments/ReceiveAttachments'
import paymentSupply from '../chain_supply/ReceiveSupplyRegister'
...
export default {
  ...
  components: {
    ...
    dataAnalyse,  
    // appNav, 
    Location, Contract, Order, Settlement, SettlementContract, Attachments, paymentSupply, ring  
  },
  data () {
    return {
      dataTime: 0,
  ...
  methods: {
    ...
    getTickets (id) {
      this.$ajax.get(`/api/chain/${id}/ticket_bar/`).then(res => {
    ...
    sureConfirm () {
      this.$confirm('此操作将向平台提交数据?', '提示', {
        ...
        this.$ajax.patch(`/api/chain/${this.$route.query.id}/verified/`).then(res=>{
          if (res.status === 200) {
            this.$message.success('操作成功!')
            // this.$refs.refContractRece.getContractInfo(this.chainId, 1)
            // this.$refs.refContractRece.getContractInfo(this.chainId, 2)
          }
    ...
    handleScroll: function () {
      ...
      _old = (that.elementScorllTop > 75) ? true : false
      if (_old !== that.element) {
        that.element = _old
        that.$store.commit('chain/changeScrollBarStatus', _old)
      }
    },
    ...
    updataSettlementId: function(e) {
      e.type === `1` ? this.settleIds.receive = e.id : this.settleIds.payment = e.id
    },
  ...
  beforeCreate: function () {
    ...
    if (that.$route.query.bid) {
      that.chainId = that.$route.query.bid
    }
    that.location = [
      {title: '企业记账', url: ''},
      {title: '总览'}
  ...
  mounted: function () {
    var that = this
    if (that.$route.query.sel) {
      this.father = {
        selIndex: Number(that.$route.query.sel)
      }
    } else {
      this.father = {
        selIndex: 0
...
.chain-diamond {
  ...
  // animation: diamondBright 1s ease-in-out 0s infinite;
  // -moz-animation: diamondBright 1s ease-in-out 0s infinite;
  // -webkit-animation: diamondBright 1s ease-in-out 0s infinite;
}
.tab-menu-group {
