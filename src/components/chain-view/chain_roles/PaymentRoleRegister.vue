          ...
          <div class="related_list_top">
            <!-- <div class="item_card" v-if="sContract[index].length !== 0">
              ...
              <div class="item_table" >
                ...
                <div v-for="(one, trIndex) in sContract[index]" :key="trIndex" v-if="trIndex === 0">
                ...
                <div class="item_table" v-for="(one, trIndex) in selContract[index]" :key="trIndex">
                  <div v-for="(log, logIndex) in one" :key="logIndex">
                    ...
                    <div class="table_line">
                      ...
                      <div><span @click="Authentication(log.auth_role)">{{log.auth_role ? log.auth_role : 'CFCA'}}</span><span></span></div>
                      <div>
                        <span class="person-link__b" @click="getAgentInfoById(v.id, v.name)" v-for="(v, i) in log.agent_list" v-if="i===0" :key="i">{{v.name}}</span>
                        <span class="person-link__b" @click="getAgentInfoById(v.id, v.name)" v-for="(v, i) in log.agent_list" v-if="i > 0" :key="i"><i>, </i>{{v.name || ''}}</span>
                      </div>
                    </div>
                  </div>
                </div>
