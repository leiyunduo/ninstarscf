<template>
  <div class="file_sign">
    ...
    <div class="inner_tip">
      ...
      <a href='/static/file/个人无限法人连带责任函.doc' target='_blank'>下载模板</a>
    </div>
    ...
    <div class="btn">
      <button @click="handleModal()" :style='file ? "" : "background: #ccc; border-color: #ccc"'>提交</button>
    </div>
    <el-dialog
...
export default {
  ...
  methods: {
    handleUpload (e) {
      ...
      let file = {}
      let loading = {}
      if (e.target.files[0].name) loading = this.$loading(this.$setLoading())
