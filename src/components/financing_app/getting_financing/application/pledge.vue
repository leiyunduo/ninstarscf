  ...
  <div class="order_usage">
    <div class="demand">
      ...
      <div class="show_list">
        <div class="get_sum">
          ...
          <button class="btn_create" @click="handleModel"><i class="el-icon-plus"></i>添加应收质押款</button>
        </div>
        <div v-for="(item, index) in selList" v-if="selList.length !== 0" :key='index' class="open_sel_item sel_item_list">
          <div class="list">
            ...
            <ul class="order_info" v-if="item.slide" v-for="(settlement, index1) in item.settlement" :key='index1'>
              <li v-for="(order, index2) in settlement.order_list" :key='index2' v-if="order.sel">
                <div class="order_data" style="border-bottom: 1px dashed #e1e6f0">
                  <div>
                    <p>订单名称：</p>
    ...
    <div class="demand">
      ...
      <div class="input_list">
        ...
        <div class="inpit_lint">
          <span>开户行：</span>
          <input type="text" placeholder="请输入开户行" v-model="bankInfo">
        </div>
      </div>
    </div>
    <div class="btn_list">
    ...
    <el-dialog :visible.sync="showModal" width="1278px">
      <div v-for="(item, index) in checkList" :class="{open_class: item.slide === 1}" v-if="checkList.length !== 0 && item.show" :key='index' class="open_sel_item">
        ...
        <ul class="order_info" v-if="item.slide" v-for="(settlement, index1) in item.settlement" :key='index1'>
          <li v-for="(order, index2) in settlement.order_list" :key='index2'>
            <div class="order_data" style="border-bottom: 1px solid #eee;">
              <div>
                <p>订单名称：</p>
                <span>{{order.name}}</span>
              </div>
              ...
              <div class="order_data_btn">
                <button @click="handleOrderSelect(index, index1, index2, settlement)">{{!order.sel ? '选择' : '取消'}}</button>
...
export default {
  ...
  mounted () {
    ...
    that.$ajax.get('/api/financing/r_first_repayment/').then(res => {
      res.data.forEach(val => {
        val.sel = false
        val.show = true
        val.slide = true
        if (val.settlement) {
          val.settlement.forEach((el, index1) => {
            el.sel = false
  ...
  methods: {
    ...
    handleShortage (arr, sw) {
      const that = this
      let s = 0
      let obj = {settlement_id_list: [], order_id_list: [], payment_amount_sum: 0}
      if (arr.length !== 0) {
        arr.forEach(val => {
          val.settlement.forEach(el => {
            if (el.sel) {
              obj.settlement_id_list.push(el.id)
              obj.payment_amount_sum += el.payment_amount
            }
          })
        })
        if (that.$parent.steps[0].emitData.length !== 0) {
          obj.chain_id = that.$parent.steps[0].emitData[0].chain_id
          that.$parent.steps[0].emitData.forEach(val => {
            val.showOrder.forEach(order => {
              obj.order_id_list.push(order.id)
            })
          })
        }
        that.$ajax.put('/api/financing/r_limit_value/', obj).then(res => {
          that.shortage = that.toThousands(res.data.limit_value)
        })
      } else {
        that.shortage = '0.00'
      }
    ...
    // },
    // 点击选择订单
    handleOrderSelect (indexContract, indexSettlement, indexOrder, settlement) {
      const that = this
      let checkList = that.checkList
      checkList[indexContract].isSel ? checkList[indexContract].isSel = false : checkList[indexContract].isSel = true
      checkList[indexContract].settlement[indexSettlement].sel ? checkList[indexContract].settlement[indexSettlement].sel = false : checkList[indexContract].settlement[indexSettlement].sel = true
      checkList[indexContract].settlement[indexSettlement].order_list.forEach(val => {
        val.sel ? val.sel = false : val.sel =true
      })
    ...
    handleSelect () {
      ...
      check.forEach(val => {
        ...
        val.settlement.forEach(el => {
          el.order_list.forEach(order => {
            if (order.sel) {
              settlementArr.push(el)
            }
          })
        })
        if (settlementArr.length !== 0) arr.push(currentContract)
      })
      that.selList = arr
      that.showModal = false
      that.handleShortage(arr)
      // return true
    },
    // 从外部取消选择
    handleDelete (index, index1, el) {
      ...
      let checkList = that.checkList
      if (selList[index].settlement.length === 1) {
        selList.splice(index, 1)
      } else {
        selList[index].settlement.splice(index, 1)
      }
      let delId = el.id
    ...
    handleNext () {
      ...
      if (that.selList.length === 0) {
        // 判断融资用途
        that.$message.error('请选择应收质押款')
      } else {
        if (!(/^[0-9]+(.[0-9]{1,2})?$/.test(this.amount))) {
          // 判断是否填写金额
          that.$message.error('请填写正确的融资金额')
        } else {
          if (Number(that.amount) >= that.shortage) {
            // 判断金额是否超标
