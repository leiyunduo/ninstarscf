  ...
  <div id="second_content">
    ...
    <div class="repayment">
      ...
      <div class="content_second">
        <h3>第二还款来源总额：{{secSum}}</h3>
        <div class="btn">
          <button @click="openList(0)"><i class="el-icon-plus"></i>添加第二还款来源</button>
        </div>
          ...
          <div class="div_list">
            <div class="top">
              <h4>
                ...
                <p>面额（元）：<span>{{toThousands(item.amount)}}</span></p>
              </h4>
              <i class="el-icon-delete" @click="handleDelete(index)"></i>
            </div>
            <div class="list">
              <ul class='ticket_line'>
                <li><span>销售项目：</span><p class="color_onlike" @click="handleChain(item)">{{item.contract_name}}</p></li>
                <li><span>票据类型：</span><p>{{item.type}}</p></li>
                <li><span>票据名称：</span><p>{{item.name}}</p></li>
                <li v-if="item.over_rate.s_rate"><span>逾期率：</span><p @click="handleOrder(item)">{{Number(item.over_rate.s_rate.over_rate * 100).toFixed(1) + '%'}}</p></li>
              </ul>
              <ul class='ticket_line'>
                <li><span>票据金额（元）：</span><p>{{item.amount}}</p></li>
                <li><span>汇票到期日：</span><p>{{item.payment_date}}</p></li>
                <li><span>付款企业名称（企业填写）：</span><p v-if="item.payment_info" :style="!item.payment_info.payment ? 'color: #999; text-decoration: none;':''">{{item.payment_info ? (item.payment_info.payment || '未填写') : '未填写'}}</p></li>
              </ul>
              <div class="diamond" v-if="item.diamond">
                ...
                <ul>
                  <li v-for="(nine, index2) in nines" :key='index2'>
                    <img 
                      :src="item.diamond[nine.key] ? diamondIcon : disdiamondIcon" 
                      alt=""
                      @click="handleChain(item)"
                      :style="item.diamond[nine.key] ? 'cursor: pointer' :'cursor: visiable'">
                    <p>{{nine.title}}</p>
                  </li>
                </ul>
              </div>
    ...
    <div class="other_guarantee">
      ...
      <div class="content_second">
        ...
        <div class="second_list" v-if="otherList.length !== 0">
          <div class="div_list_content" v-for="(item, index) in otherList" :key="index">
            <div class="top">
              <ul>
                <li><span>类型：</span><p>{{item.collateral_type === '1' ? '房产' : item.collateral_type === '2' ? '汽车' : item.collateral_type === '3' ? '股权' : '基金'}}</p></li>
                <li><span>市场估值：</span><p>{{toThousands(item.appraisement)}}</p></li>
                <li></li>
              </ul>
            ...
            <div class="list" v-if="item.collateral_type === '1'">
              <ul>
                ...
                <li>
                  ...
                  <span v-if="item.att_list.length !== 0 && item.att_list" class="color_onlike" v-for="(it, index1) in item.att_list" :key='index1' style="margin-rigth: 10px" @click="handlePreview(it)">{{it.name}}</span>
                </li>
              </ul>
            </div>
            <div class="list" v-if="item.collateral_type === '2'">
              <ul>
                ...
                <li>
                  <span>评估报告：</span>
                  <p v-if="item.att_list.length === 0 || !item.att_list" style="color: #999">暂无报告</p>
                  <span class="color_onlike" v-if="item.att_list.length !== 0 && item.att_list" v-for="(it, index1) in item.att_list" :key='index1' @click="handlePreview(it)">{{it.name}}</span>
                </li>
              </ul>
            </div>
    ...
    <el-dialog :visible.sync="dialogVisibles" width="1300px" :before-close="handleClose">
      <div class="el_dialog_divlist" v-if="checkList.length !== 0">
        <div class="div_list" v-for="(item, index) in checkList" :key='index' v-if="checkList !== 0">
          ...
          <div class="list">
            <ul v-if="item.over_rate" class='ticket_line'>
              <li><span>销售项目：</span><p @click="handleChain(item)">{{item.contract_name}}</p></li>
              <li><span>票据类型：</span><p>{{item.type}}</p></li>
              <li><span>票据名称：</span><p>{{item.name}}</p></li>
              <li v-if="item.over_rate.s_rate"><span>逾期率：</span><p @click="handleOrder(item)">{{Number(item.over_rate.s_rate.over_rate * 100).toFixed(1) + '%'}}</p></li>
            </ul>
            <ul class='ticket_line'>
              <li><span>票据金额（元）：</span><p>{{toThousands(item.amount)}}</p></li>
              <li><span>汇票到期日：</span><p>{{item.payment_date}}</p></li>
              <li><span>付款企业名称（企业填写）：</span><p :style="!item.payment_info ? 'color: #999; text-decoration: none;':''">{{item.payment_info ? (item.payment_info.payment || '未填写') : '未填写'}}</p></li>
            </ul>
            <div class="diamond" v-if="item.diamond">
              <div class="line"></div>
              <ul>
      ...
      <div class="null_tip" v-if="checkList.length === 0">暂无可用的应收票据。</div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      pics: ['', require('../../../../../static/img/tickets/jq_01.png'), require('../../../../../static/img/tickets/yq_01.png'), require('../../../../../static/img/tickets/tq_01.png')]
    }
  },
  created () {
    const that = this
    this.$ajax.get(`/api/financing/ticket_list/?chain_id=${this.$parent.steps[0].emitData.list[0].chain_id}`).then(res => {
      if (res.status === 200) {
