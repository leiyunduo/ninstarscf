<template>
  <div class='usage_card'>
    ...
    <div class="item_wrap">
      ...
      <div v-for="(item, index) in usage" :key='index' class='usage_item' v-if='usage.length !== 0'>
        ...
        <div v-if="item.order" class='order_info'>
          <div v-for='(order, index1) in item.order' :key='index1'>
            ...
            <financing-ring :id='item.chain_id' />
            <div class="btn_list">
      ...
      <div v-if="usage.length === 0">
        <p style="color: #999; text-align: center; line-height: 240px;">未提交融资用途</p>
      </div>
    </div>
    <!-- 九环 -->
    <el-dialog
...
export default {
  ...
  created () {
    ...
    this.$ajax.get(`/api/financing/application/${that.id}/`).then(res => {
      if (res.status === 200) {
        ...
        that.base = res.data.expect_data
        that.orderSum = res.data.total ? Number(res.data.total.purpose_total).toFixed(2) : false
        that.usage = res.data.application_data.chain_info || res.data.application_data.loan_purpose
        // 申请企业信息
        ...
        // 我的企业信息
        that.$ajax.get(`/api/company/` + localStorage.id + '?detail_info=1').then(res1 => {
          that.businessInfo = {
            id: res1.data.id,
            name: res1.data.detail_info ? res1.data.detail_info.name : null,
            open_account_code: res1.data.detail_info ? res1.data.detail_info.opening_bank_info.bank_account : null,
  ...
  methods: {
    ...
    // 跳转链式记账
    handleChain (item) {
      window.open(`#/chain_app/payment_chain?id=${item.chain_id}&financing_id=${item.id}`)
    },
    // 查看企业
    handlebusiness (item) {
    ...
    handleOrder (item) {
      ...
      window.open(`#/myFund/order_list?tar=p&partner=${partner}&company=${company}`)
    },
