<template>
  <div class="assetsList">
    <div class="bottom_list">
              ...
              <div class="el_col_list el1" v-if="item.application_data">
                ...
                <div v-if="!item.is_region_code && item.loan_process_info">
                  <ul class='institution_line'>
                    <li v-for="(loan, indexLoan) in item.loan_process_info" :key='indexLoan' class="institution_line">
                      <span>{{loan.institution_name}}</span>
                      <span @click="handleDetail(loan)" style='cursor: pointer;'>{{loan.loan_process_status_display}}</span>
                    </li>
                  </ul>                  
                </div>
              </div>
      ...
      <div v-if="showList.length === 0" class="null_application">暂无已完成的融资</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    handleAjax (page, type, page_size) {
      ...
      this.$ajax.get(`/api/financing/application/?page=${p}&page_size=${pz}&status=4${sy}`).then(res => {
        if (res.status === 200) {
          that.showList = res.data.results
          that.total = res.data.conut
        }
      })
    },
    // 格式化时间戳
