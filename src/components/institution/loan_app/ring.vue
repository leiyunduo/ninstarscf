  ...
  <div class='financing_ring'>
    <div class='ring_wrap' v-if='getting'>
      <div class='roles'>
        ...
        <p>供应商</p>
        <select @change='handleSelect($event)' v-if="hasP">
          <option :value="item.partner_company_id" v-for="(item, index) in customers" :key='index' @click="handleBusiness(item)">{{item.partner_company__name}}</option>
        </select>  
        <p v-if="!hasP">无供应商</p>
      </div>
      <ul>
        <li v-for="(item, index) in pTitle" :key='index' v-if="pPart[item.key]">
          ...
          <div class="date date_b" v-if="index === 0 && hasP">
            <p>应付账期=+{{pPart[item.key] ? pPart[item.key][0].days : 0}}天</p>
          </div>
          <div class="date date_b" v-if="index === 2 && hasP">
            <p>贷款周期={{financingDate > 0 ? '+' + financingDate : financingDate}}天</p>
          </div>
          <div class='date date_b' v-if="index === 8 && hasP">
            <p>{{paymentDate}}</p>
          </div>
          <img :src="handlePic('p', item.key, index)" alt="" :class='(index === 8 && pPart[item.key].length === 0) && paymentTime <= 7 && hasP ? "flash" : "no"'>
          <p>
    ...
    <div class='ring_wrap' v-if='getting'>
      ...
      <div class='roles'>
        ...
        <p>客户</p>
        <p @click="handleBusiness(affonder)">{{affonder.partner_company__name}}</p>      
      </div>
    </div>  
...
export default {
  ...
  methods: {
    ...
    // 跳转链式记账
    handleChain () {
      window.open('#/credit_app/chain_info?sel=2&id=' + this.id + '&bid=' + this.businessId, '_blank')
    },
    // 跳转企业博客
    handleBusiness (item) {
      console.log(item)
      let f = item.partner_company_id ? '1' : '0'
      let key = item.partner_company__detail_id ? '1' : '0'
      let bar = '3'
      window.open('#/enterpriseBlog?barIndex=' + bar + '&bid=' + item.partner_company_id + '&key=' + key + '&f=' + f, '_black')
    }
  }
}
</script>
