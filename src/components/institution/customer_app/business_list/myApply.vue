<template>
  <div id="myApply">
    ...
    <div class="myApply_list">
      <!-- <div class="navs">
        <h3><img src="static/img/leftbar/a1.png" alt=""><span @click="nextClick">区域企业</span><i class="el-icon-arrow-right"></i><span>成员申请</span></h3>
      </div> -->
      ...
      <div class="list_content">
        <div class="content">
            ...
            <el-row>
              ...
              <el-col :xs="10" :sm="10" :md="9" :lg="10" :xl="10">
                <div class="center">
                  ...
                  <div class="textarea" v-else>对方企业请求加入融资</div>
              ...
              <el-col :xs="7" :sm="7" :md="6" :lg="7" :xl="7">
                <div class="right">
                  ...
                  <div class="accept_refuse" v-if="item.status === '0'">
                    <button @click="acceptClick(item, index)">审核</button>
          ...
          <div v-if="businessData.length === 0" class="null_apply">暂无客户申请</div>
          <div class="block">
    ...
    <el-dialog align='center' :visible.sync="managementVisible" width="570px" :before-close="handleClose">
      ...
      <el-dialog width="500px" title="融资授信确认方式选择" :visible.sync="innerVisible" append-to-body>
...
export default {
  ...
  mounted () {
    const that = this
    that.$ajax.get('/api/institution/customer/?status=0&page=' + this.currPage + '&page_size=' + this.pageSize).then(res => {
  ...
  methods: {// 分页
    handleSizeChange (val) {
      const that = this
      that.currPage = val
      that.$ajax.get('/api/institution/customer/?type=0&status=0&page=' + this.currPage + '&page_size=' + this.pageSize).then(res => {
        that.businessData = res.data.results
        that.total = res.data.count
      }).catch(err => {
        that.$message.error(err.response.data.err_msg)
    ...
    nextClick () {
      const that = this
      that.$router.push('/customer_app')
    },
    // 接受
    acceptClick (item, index) {
      // const that = this
      console.log(item)
      let id = item.business_info.id
      window.open('#/enterpriseBlog?auth=1&bid=' + id + '&aid=' + item.id, '_black')
      // that.$ajax.put(`/api/institution/customer/${item.id}/`, {status: 1}).then(res => {
      //   if (res.status === 200) {
    ...
    refuseOK () {
      const that = this
      if (that.res_msg !== '') {
        that.$ajax.put(`/api/institution/customer/${that.refuse_id}/`, {status: 2, res_msg: that.res_msg}).then(res => {
          if (res.status === 200) {
            that.$message.success('接受成功')
            that.$ajax.get('/api/institution/customer/?type=0&status=0&page=' + this.currPage + '&page_size=' + this.pageSize).then(res => {
              that.businessData = res.data.results
    ...
    managementOK () {
      ...
      if (that.form.group === '1') {
        ...
        if (that.form.total_limit === '') {
          that.$message.error('请输入融资融资授信总额度')
        } else {
          if (that.form.single_limit === '') {
            that.$message.error('请输入单笔融资融资授信额度')
          } else {
            if (Number(that.form.total_limit) < Number(that.form.single_limit)) {
              that.$message.error('融资融资授信总额度不能小于单笔融资融资授信额度')
            } else {
