...
export default {
  ...
  components: {
    Location, acceptanceList, navBar, 
    // AppNav
  },
  beforeCreate () {
    var that = this
    ...
    this.location = [
      ...
      {
        title: '链式记账详情',
        url: Number(this.chain_type) === 1 ? '/credit_app/chain_detail?id=' + this.$route.query.chain_id + '&id_list=' + that.$route.query.id_list : '/credit_app/chain_info?id=' + this.$route.query.chain_id + '&id_list=' + that.$route.query.id_list
      },
      {
        title: '查看验收单'
      }
