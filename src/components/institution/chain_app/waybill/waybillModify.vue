...
export default {
  ...
  components: {
    Location, AppNav, waybillList, 
    // navBar
  },
  created () {
    var that = this
    that.only = that.$route.query.only
    ...
    this.location = [
      ...
      },
      {
        ...
        ?
        '/credit_app/chain_detail?id=' + this.$route.query.chain_id + '&id_list=' + that.$route.query.id_list
        :
        '/credit_app/chain_info?id=' + this.$route.query.chain_id + '&id_list=' + that.$route.query.id_list
      },
      {
        title: '查看运单'
      }
