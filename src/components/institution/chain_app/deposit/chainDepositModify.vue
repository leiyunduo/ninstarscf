        ...
        <div class="chain-deposit__form chain_flex">
          <div class="chain-deposit__form__group chain-width-r3">
            <div class="chain-deposit__form__row">
              ...
              <input type="text" maxlength="32" :disabled="true" v-model="deposit.pay_name">
            </div>
            <div class="chain-deposit__form__row chain-from_select">
              <span class="chain-required-orange">*</span>
              <label for="">支付方式:</label>
              <el-select size="small" v-model="deposit.pay_type" :disabled="isDisabled.pay_type" @change="changeTab(deposit.pay_type)" placeholder="请选择支付方式">
            ...
            <div class="chain-deposit__form__row">
              ...
              <el-date-picker size="small" class="chain-box_warning" type="date" :disabled="isDisabled.pay_date" v-model="deposit.pay_date" placeholder="选择日期" format="yyyy-MM-dd" value-format="yyyy-MM-dd"></el-date-picker>
            </div>
            <div class="chain-deposit__form__row" v-if="Number(deposit.pay_type) > 2">
              <span class="chain-required-orange">*</span>
              <label for="">汇票到期日:</label>
              <el-date-picker size="small" type="date" :disabled="isDisabled.pay_deadline" v-model="deposit.pay_deadline" placeholder="选择日期" format="yyyy-MM-dd" value-format="yyyy-MM-dd"></el-date-picker>
            </div>
          </div>
          <div class="chain-deposit__form__group chain-width-r3" v-if="Number(deposit.pay_type) !== 1">
            ...
            <div class="chain-deposit__form__row">
              ...
              <input class="maxLength" type="text" maxlength="32" :disabled="isDisabled.receive_account" v-model="deposit.receive_account">
            </div>
          </div>
          <div class="chain-deposit__form__group chain-width-r3" v-if="Number(deposit.pay_type) !== 1">
            <div class="chain-deposit__form__row">
            ...
            <div class="chain-deposit__form__row">
              ...
              <input class="maxLength" type="text" maxlength="32" :disabled="isDisabled.pay_account" v-model="deposit.pay_account">
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  props: ['contractId', 'orderId', 'chainId', 'chainType', 'depositId'],
  data () {
    return {
      project_status: '',
      step: {selIndex: 1},
  ...
  methods: {
    ...
    sureCommit () {
      ...
      let params = {
        ...
        payment_date: this.deposit.pay_deadline || null,
      ...
      this.depositModify(this.prev_id, params)
    },
    ...
    //     this.deposit = this.copyObject(this.deposit_cache)
    //   }
    ...
    addChanges () {
      ...
      this.showIndex = this.btnList.length - 1
      for (let item in this.deposit) {
        if (['attach_list'].includes(item)) {
          this.deposit[item] = []
        } else if (item !== 'pay_name') {
          this.deposit[item] = ''
        }
      }
  ...
  watch: {
    deposit: {
      handler (newVal, old) {
        if (this.showIndex === this.btnList.length - 1) {
          this.deposit_cache = this.copyObject(newVal)
        }
      },
      deep: true
    },
