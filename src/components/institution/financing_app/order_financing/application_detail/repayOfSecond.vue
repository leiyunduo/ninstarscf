      ...
      <ul class="cover_pool_list" v-if="showList.length !== 0">
        <li v-for="(item, index) in showList" :key='index'>
          ...
          <div class="show_item_infoline">
            <div>
              <span>销售项目：</span>
              <span style="cursor: pointer; text-decoration: underline; color: #eb6100; " @click="handleChain(item)">{{item.contract_name}}</span>
            </div>
            <div>
              <span>付款人：</span>
            ...
            <div class="div_list_content" v-if="otherList.length !== 0" v-for="(item, index) in otherList" :key="index">
              ...
              <div class="list" v-if="item.collateral_type === '1'">
                <ul>
                  ...
                  <li><span>评估报告：</span><p v-if="item.att_list.length === 0 || !item.att_list" style="color: #999">暂无报告</p><p class="color_onlike" @click="handleFileDetail(it)" v-if="item.att_list.length !== 0 && item.att_list" v-for="(it, index1) in item.att_list" :key='index1'>{{it.name}}</p></li>
                </ul>
              </div>
...
export default {
  ...
  beforeMount () {
    ...
    that.$ajax.get(`/api/financing/loan_process/${that.$route.query.financing_id}`).then(res => {
      if (res.status === 200) {
        that.showList = res.data.financing_application.second_repayment
        that.otherList = res.data.financing_application.application_data.other_collateral
      }
    })
  },
  methods: {
    handleChain (item) {
      ...
      if (item.id_list) {
        let idList = ''
        item.id_list.forEach(val => {
          idList += `${val},`
        })
        idlist = '&id_list=' + idList.substring(0, idList.length - 1)
      }
      window.open(str + item.chain_id + idlist)
    },
    handleFileDetail (it) {
      window.open(this.$root_url() + it.url, '_blank')
    }
  }
}
</script>
