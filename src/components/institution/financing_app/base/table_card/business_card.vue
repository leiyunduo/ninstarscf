<template>
  <div class='business_card' :style="modal ? 'margin-top: -20px' : ''">
    <div class="card_title">申请企业信息</div>
    ...
    <div class="information_card">
      ...
      <div class='information_line'>
        <div class="information_item">
          <span>营业执照注册号：</span>
          <span :class='businessInformation.business_license_id ? "" : "null"'>{{businessInformation.business_license_id || '企业未登记'}}</span>
        </div>
        <div class="information_item">
          <span>经营期限：</span>
      ...
      <div class='information_desc'>
        <span>经营范围：</span>
        <p :style='businessInformation.scope ? "" : "color: #999"'>{{businessInformation.scope || '企业未填写'}}</p>
      </div>
    </div>
...
<script>
import robot from '../../../area_app/asset/robot.vue'
  ...
  created () {
    ...
    that.$ajax.get(`/api/company/` + that.id + '?detail_info=1').then(res => {
      ...
      that.businessInformation = {
        ...
        // 营业执照代码
        business_license_id: res.data.detail_info.business_information.no,
        // 注册资本
