<template>
  <div class='id_card' :style="modal ? 'margin-top: -20px' : ''">
    ...
    <div class="web">
      ...
      <button @click="handleTalk()"><i class='el-icon-message'></i>银企交流</button>
    </div>
...
export default {
    ...
    that.$ajax.get(`/api/financing/loan_process/${that.id}`).then(res => {
      if (res.status === 200) {
        ...
        that.pic = {
          ...
          usage: that.handleShotage(res.data.financing_application.application_data.first_repayment) * 0.7,
          first_sum: that.handleSum(res.data.financing_application.application_data.first_repayment),
          shortage: that.handleShotage(res.data.financing_application.application_data.chain_info)
        }
        that.sum = {
          usage: that.handleSum(res.data.financing_application.application_data.chain_info),
          first: that.handleSum(res.data.financing_application.application_data.first_repayment),
          second: that.handleSum1(res.data.financing_application.second_repayment),
          other: that.handleSum1(res.data.financing_application.application_data.other_collateral)
        }
  ...
  methods: {
    ...
    handleBusiness (el) {
      ...
      // @key has_info
      let f = el.has_partner_company ? '1' : '0'
      let key = el.has_info ? '1' : '0'
