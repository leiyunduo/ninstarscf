...
export default {
  ...
  created () {
    ...
    that.$ajax.get(`/api/financing/loan_process/${that.id}`).then(res => {
      if (res.status === 200) {
        if (res.data.financing_application.fund_info) {
          if (res.data.financing_application.helping_status === '0' || res.data.financing_application.helping_status === '2' || res.data.financing_application.helping_status === '5') {
            that.info = {
              name: res.data.financing_application.fund_info.name,
              helping_status: res.data.financing_application.helping_status,
              ...res.data.financing_application.member_info,
              rate: that.fundList[res.data.financing_application.helping_status].title,
              amount: that.fundList[res.data.financing_application.helping_status].title,
              hasRate: false
            }
          } else {
            that.info = {
