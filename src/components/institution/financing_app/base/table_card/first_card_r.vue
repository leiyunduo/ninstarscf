    ...
    <div class="item_wrap">
      ...
      <div v-for="(item, index) in usage" :key='index' class='usage_item'>
        ...
        <div v-if="item.settlement" class='order_info'>
          <div v-for="(settlement, sindex) in item.settlement" :key='sindex'>
            <div v-for='(order, index1) in settlement.order_list' :key='index1'>
              ...
              <div>
                <financing-ring :id='uid'></financing-ring>
              </div>
    ...
    <el-dialog
      ...
      <div class="five_item" v-if="fives.channel === 0">
        ...
        <div class="show_item_line" v-if="fives.att_list">
          ...
          <div>
            <ul v-if="fives.att_list.length !== 0" class="inner_file">
              <li>
                <div>文件名称</div>
                <div>查看</div>
              </li>
...
export default {
  ...
  created () {
    ...
    that.$ajax.get(`/api/financing/loan_process/${that.id}`).then(res => {
      if (res.status === 200) {
        ...
        that.usage = res.data.financing_application.first_repayment
        that.orderSum = res.data.financing_application.total ? Number(res.data.financing_application.total.first_repayment_total).toFixed(2) : false
        that.aid = res.data.financing_application.business
  ...
  methods: {
    ...
    handleOrder (item) {
      ...
      window.open(`#/credit_app/order_list?partner=${partner}&company=${company}&tar=s`)
    },
    // 计算五流
