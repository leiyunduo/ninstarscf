<template>
  <div class='usage_card'>
    <div class="card_title">融资用途</div>
    <div class='usage_slide'>
      <div v-if="orderSum">
        <span>采购应付总额（元）：</span>
        <span>{{toThousands(orderSum)}}</span>
      </div>
      <div>
        <span>期望融资金额（元）：</span>
      ...
      <div v-for="(item, index) in usage" :key='index' class='usage_item'>
        <div class='contract_info'>
          ...
          <div v-if="item.over_rate">
            <span>历史逾期率：</span>
            <span @click="handleOrder(item)" v-if="item.over_rate.p_rate"><i class="el-icon-bell"></i>{{item.over_rate.p_rate.over_rate ? Number(item.over_rate.p_rate.over_rate * 100).toFixed(1) + '%' : '0.0%'}}</span>
          </div>
        </div>
        <div v-if="item.order" class='order_info'>
          <div v-for='(order, index1) in item.order' :key='index1'>
    ...
    <!-- 个人 -->
    <person v-if="dialogVisiblePerson" @closeView='dialogVisiblePerson = false' :pid="pid" :pName="pName" :isAgent="isAgent" :top='true' :use='false'></person>
  </div>
</template>
<script>
...
export default {
  ...
  created () {
    ...
    that.$ajax.get(`/api/financing/loan_process/${that.id}`).then(res => {
      if (res.status === 200) {
        that.infoId = res.data.financing_application.business
        that.base = res.data.financing_application.expect_data
        that.orderSum = res.data.financing_application.total ? Number(res.data.financing_application.total.purpose_total).toFixed(2) : false
        that.usage = res.data.financing_application.application_data.chain_info || res.data.financing_application.application_data.loan_purpose
        that.uid = res.data.id.toString()
        if (that.usage.length !== 0) {
          // 合作企业信息
  ...
  methods: {
    ...
    // 跳转链式记账
    handleChain (item) {
      window.open(`#/credit_app/chain_info?id=${item.chain_id}&financing_id=${item.id}`)
    },
    // 查看企业
    handlebusiness (item) {
    ...
    handleOtherBusiness (id) {
      if (id) window.open('#/enterpriseBlog?bid=' + id + '&barIndex=2&debt=1', '_black')
    },
    // 查看历史记账
    handleOrder (item) {
      const that = this
      ...
      window.open(`#/credit_app/order_list?tar=p&partner=${partner}&company=${company}`)
    },
    handleOrderView (item) {
      // window.open(`#/chain_app/order?contract_id=${item.contract_id}&chain_id=${item.chain_id}&chian_type=${item.chain_type}&order_id=${item.id}`)
    },
