  ...
  <div class='id_card' :style="modal ? 'margin-top: -20px' : ''">
    ...
    <div class="pic_info">
      ...
      <div>
        ...
        <p>
          <a target='blank' :href="base.businessInfo ? `#/enterpriseBlog?bid=${base.businessInfo.bid}` : '#'">{{base.businessInfo ? base.businessInfo.name : '链主'}}</a></p>
      </div>
      <div class="line">
        <div></div>
        <div>
    ...
    <div class="show_list">
      ...
      <div>
        ...
        <div class="null_tip" v-if="firstRepayment.length === 0">未提交质押池</div>
        <!-- 显示印章 -->
        <p>
          <span>助贷增信信息</span>
        </p>
            ...
            <li v-if="fund">
              <div class="sign_detail">
                ...
                <div class="info_card">
                  ...
                  <div>
                    <span>助贷占比：</span>
                    <span>{{fund.show_rate ? + fund.show_rate + '%' : '审核中'}}</span>
                  </div>
                </div>
                <div class="info_card">
                  <div>
...
export default {
  ...
  created () {
    ...
    that.$ajax.get(`/api/financing/loan_process/${that.id}`).then(res => {
      if (res.status === 200) {
        if (!res.data.financing_application.is_helping) {
          that.fund = null
        } else {
          that.fund = {
            status: res.data.financing_application.helping_status,
            ...res.data.financing_application.helping_data,
            ...res.data.financing_application.member_info,
            ...res.data.financing_application.fund_info,
            show_rate: !res.data.financing_application.helping_data.rate ? res.data.financing_application.member_info.rate : res.data.financing_application.helping_data.rate
          }
        }
        that.base = {
          financing_id: res.data.financing_application.id,
          businessInfo: {
            name: res.data.financing_application.business_info.name,
            uid: res.data.financing_application.business_info.uid,
            bid: res.data.financing_application.business_info.id
          },
          ...res.data.financing_application.business_info.contact_info,
          ...res.data.financing_application.expect_data,
          file: res.data.financing_application.application_data.file,
          create_time: res.data.financing_application.create_time
        }
        that.bank = res.data.financing_application.application_data.bank_account_info || {}
        that.pic = {
          value: res.data.financing_application.expect_data.value,
          usage: that.handleShotage(res.data.financing_application.application_data.loan_purpose) * 0.7,
          first_sum: that.toThousands(res.data.financing_application.total.first_repayment_total),
          shortage: that.handleShotage(res.data.financing_application.first_repayment)
        }
        that.sum = {
          usage: that.toThousands(res.data.financing_application.total.purpose_total),
          first: that.toThousands(res.data.financing_application.total.first_repayment_total),
          second: that.toThousands(res.data.financing_application.total.second_repayment_total),
          other: that.toThousands(res.data.financing_application.total.other_collateral_total)
        }
        that.usage = res.data.financing_application.application_data.loan_purpose
        ...
        that.otherRepayment = res.data.financing_application.application_data.other_collateral
        that.ninstar = res.data.financing_application.helping_data.ninstar_info
        that.file = res.data.financing_application.application_data.file
      }
    })
  },
  methods: {
    ...
    handleOrder (item, r, tar) {
      ...
      window.open(`#/credit_app/order_list?partner=${partner}&company=${company}&tar=${tar}`)
    ...
    handleChain (el) {
      let query = {}
      let path = '#/credit_app/loan_purpose'
      if (el.chain_type) {
        el.chain_type === 2 ? path = '#/credit_app/loan_purpose' : path = '#/credit_app/chain_detail'
      }
    ...
    // 查看企业首页
    handleBusiness (el) {
      let f = item.has_partner_company ? '1' : '0'
      let key = item.has_info ? '1' : '0'
      let bar = this.$route.name === 'creditIndex' ? '1' : '2'
      window.open('#/enterpriseBlog?barIndex=' + bar + '&bid=' + item.partner_company + '&key=' + key + '&f=' + f, '_black')
    },
    // 查看文件
    handleFile () {
      const that = this
