  ...
  <div class='usage_card'>
    ...
    <div class="second">
      ...
      <div class="second_wrap">
        ...
        <div>
          <div v-for="(item, index) in second" :key='index' class='usage_item'>
            ...
            <div class="order_title" v-if="item.partner_info">
              <div>
                <span>客户：</span>
                <span @click="handleBusiness(item)" style="color: #eb6100; text-decoration: underline">{{item.partner_company_name}}</span>
              </div>
              <div>
                <span>营业执照号码： </span>
        ...
        <div>
          <div v-for="(item, index1) in ticket" :key='index1' class='usage_item'>
            <div class='info_title'>
              <div>
                <img :src="tickets[item.ticket_type].icon" alt="" v-if="item.ticket_type">
                <span>{{tickets[item.ticket_type].title}}面额（元）：</span>
                <span>{{toThousands(item.amount)}}</span>
              </div>
            </div>
            <div class='contract_info'>
              ...
              <div v-if="item.over_rate">
                <span>历史逾期率：</span>
                <span v-if="item.over_rate.s_rate"><i class="el-icon-bell"></i>{{(Number(item.over_rate.s_rate.over_rate) * 100).toFixed(1) + '%' || '0%'}}</span>
              </div>
            </div>
            <div class='order_title'>
              <div>
            ...
            <div class='diamond' v-if="item.diamond">
              <div class="line"></div>
              <ul>
                <li v-for="(diamond, key, index3) in item.diamond" :key='index3'>
                  <img :src="diamond ? diamondIcon : disdiamondIcon" alt="">
                  <p>{{nines[index3]}}</p>
                </li>
              </ul>
            </div>
...
export default {
  ...
  created () {
    ...
    that.$ajax.get(`/api/financing/loan_process/${that.id}`).then(res => {
      if (res.status === 200) {
        let n = res.data.financing_application.second_repayment
        let second = []
        let ticket = []
        n.forEach((val, index) => {
          if (val.ticket !== '3') {
  ...
  methods: {
    ...
    handleOrder (item, r) {
      ...
      let company = that.businessInfo.id
      // window.open(`#/credit_app/order_list?partner=${partner}&company=${company}`)
    ...
    handleDetail (item) {
      let url = ''
      item.chain_type === '2' ? url = '#/credit_app/chain_info?id=' : url = '#/credit_app/chain_detail?id='
