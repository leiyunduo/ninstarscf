...
export default {
  ...
  methods: {
    handleAjax (page, page_size, type) {
      ...
      this.$ajax.get(`/api/financing/loan_process/?is_active=1&loan_process_status=101&page=${page}&page_size=${pz}&financing_application__type=0`).then(res => {
        if (res.status === 200) {
          let showList = []
    ...
    handleRefuse (item) {
      const that = this
      that.$confirm('拒绝后，本次融资结束', '拒绝申请', {type: 'error'}).then(res => {
