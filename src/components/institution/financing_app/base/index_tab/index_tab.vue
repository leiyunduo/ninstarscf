<template>
  <div class="assetsList">
    <div class="bottom_list">
      <div v-for="(item, index) in showList" :key='index'>
        <div class="bottom_top">
          <div class="left">
            ...
            <p>融资编号：<span v-if="item.financing_application.id" @click='handlePic(item.id)' class="show_id">{{item.financing_application.id}}</span></p>
            <p>提交时间：<span>{{item.create_time}}</span></p>
          </div>
          <div class="right">
            <button class="install" @click="handlePass(item.id)">通过</button>
            <button class="refuse" @click="handleRefuse(item.id)">拒绝</button>
          </div>
        </div>
                ...
                <ul v-if="item.application_data">
                  ...
                  <li v-for="(el, indexChan) in item.application_data.chain_info" :key='indexChan' v-if="item.application_data.chain_info" class="order_line">
                    ...
                    <span v-if="el.order" v-for="(order, i) in el.order" :key='i'>
                      <div><span :title='order.name' style="color: #eb6100;" @click="handleChain(el, item)">{{order.name}}</span><span>{{toThousands(order.amount)}}（元）</span></div>
                    </span>
                  </li>
                </ul>
              </div>
            ...
            <el-col :xs="4" :sm="4" :md="4" :lg="4" :xl="4">
              <div class="el_col_list el6  right_no">
                ...
                <ul v-if="item.second_repayment" v-for="(sec,secIndex) in item.second_repayment" :key='secIndex'>
                  <li><span>其他应收：</span><span :title="sec.name" @click="handleChainSel(sec)" style="color:#e87626; text-decoration: underline;cursor: pointer">{{sec.name}}</span></li>
      ...
      <div v-if="showList.length === 0" class="null_application">暂无融资申请</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    handleAjax (page, page_size, type) {
      ...
      this.$ajax.get(`/api/financing/loan_process/?is_active=1&loan_process_status=100&page=${page}&page_size=${pz}&type=${sy}`).then(res => {
        console.log(res)
        if (res.status === 200) {
