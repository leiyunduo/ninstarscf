<template>
  <div class="assetsList">
    <div class="bottom_list">
      <button class="posBtn" @click="handlePass()" v-if="showList.length !== 0">统一发送到风控</button>
      <div v-for="(item, index) in showList" :key='index'>
        <div class="bottom_top">
          <div class="left">
            ...
            <p>通过类型：<span>{{item.pass_type || '人工审核通过'}}</span></p>
          </div>
          <div class="right">
            <button class="install" @click="handlePreview(item)">预览风控内容</button>
            <button class="install" @click="handleSigle(item)">发送到风控</button>
            <button class="refuse" @click="handleRefuse(item)">拒绝申请</button>
          </div>
        </div>
      ...
      <div v-if="showList.length === 0" class="null_application">暂无通过预审的融资申请</div>
    </div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    handleAjax (page, page_size, type) {
      ...
      let p = page || 1
      let pz = page_size || 5
      this.$ajax.get(`/api/financing/loan_process/?is_active=1&loan_process_status=101&page=${page}&page_size=${pz}&type=${sy}`).then(res => {
        console.log(res)
        if (res.status === 200) {
    ...
    handlePreview (item) {
      ...
      if (item.pdf_url) {
        window.open(that.$root_url() + item.pdf_url, '_blank')    
      } else {
          that.$message.warning('生成中，请稍后重试')
      }
    },
    // 预审通过
    handlePass () {
      ...
      if (arr.length === 0) {
        ...
        that.handleSend(arr, loading)
      }
    },
    handleSend (loan_process_id_list, loading) {
      const that = this
      console.log(loan_process_id_list)
      that.$ajax.put(`/api/financing/loan_process/send_risk_control/`, {loan_process_id_list}).then(res => {
    ...
    handleRefuse (item) {
      ...
      that.$confirm('拒绝后，本次融资结束', '拒绝申请', {type: 'warning'}).then(res => {
        that.$ajax.put(`/api/financing/loan_process/${item.id}/`, {loan_process_status: 202}).then(res => {
          if (res.status === 200) {
