...
export default {
  ...
  methods: {
    handleAjax (page, page_size, type) {
      ...
      this.$ajax.get(`/api/financing/loan_process/?is_active=1&loan_process_status__in=600,601,602,700,701&page=${page}&page_size=${pz}&financing_application__type=0`).then(res => {
        console.log(res)
        if (res.status === 200) {
          let showList = []
