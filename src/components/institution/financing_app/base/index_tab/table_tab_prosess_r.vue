...
export default {
  ...
  methods: {
    handleAjax (page, page_size, type) {
      ...
      this.$ajax.get(`/api/financing/loan_process/?is_active=1&loan_process_status__in=200,201,300,301,400,401,500,501,502&page=${page}&page_size=${pz}&financing_application__type=0`).then(res => {
        console.log(res)
        if (res.status === 200) {
          let showList = []
    ...
    // 计算利率值
    handleRate (obj) {
      if (obj.repayment_rate) {
        return Number(obj.repayment_rate).toFixed(3) + '%'
      } else {
        if (obj.loan_info) {
          if (obj.loan_info.expect_rate) {
            return obj.loan_info.expect_rate + '%'
          } else {
            return '未填写'
          }
        } else {
