<template>
  <div class="assetsList">
    ...
    <el-dialog
      ...
      title= '您将拒接本次贷款申请，拒绝后不可撤销？'>
      <div class='button_list'>
        <button @click="handleRefuse">确定</button>
...
export default {
  ...
  methods: {
    handleAjax (page, ps, type) {
      ...
      this.$ajax.get(`/api/financing/loan_process/?is_active=1&loan_process_status=100&page=${p}&page_size=${pz}&financing_application__type=1`).then(res => {
        if (res.status === 200) {
    ...
    handleDetail (event, index, index1) {
      ...
      if (showReg.includes(index1)) {
        ...
        console.log(el.financing_application.second_repayment)
        console.log(index1)
        if (item.length !== 0) {
          item.forEach(val => {
