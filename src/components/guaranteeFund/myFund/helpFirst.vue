<template>
  <div class="helpFirst">
    <div class="helpFirst__box">
      <!-- <appNav :father="step"></appNav> -->
      <div class="helpFirst__box__wrap">
        <div class="top-nav">
          <location :location="location"></location>
        </div>
        <div class="helpFirst__box__inner">
...
export default {
  ...
  methods: {
    ...
    handleChain (el) {
      ...
      query.financing_id = this.$route.query.chain_id
      window.open(`#/fund/chain_info?id=${query.id}&financing_id=${query.financing_id}`)
    },
    // 点击企业名称跳转博客
    businessBlog (item) {
      const that = this
      window.open('#/enterpriseBlog?bid=' + item.partner_company + '&help=2' + '&chain_id=' + that.$route.query.chain_id)
    },
    // 项目订单总和
    handleSum () {
      let sun = 0
