<template>
  <div class="helpPurpose">
    <div class="helpPurpose__box">
      <!-- <appNav :father="step"></appNav> -->
      <div class="helpPurpose__box__wrap">
        <div class="top-nav">
          <location :location="location"></location>
        </div>
        <div class="helpPurpose__box__inner">
          <div class="financing-item">
            ...
            <div class="financing-item__body" v-for="(item, index) in showList" :key='index'>
              <p class="financing-item-desc">
                采购项目：<span style="cursor:pointer;" @click="handleChain(item)">{{item.name}}</span>
                销售企业：<span style="cursor:pointer;" @click="businessBlog(item)">{{item.partner_company_name}}</span>
              </p>
              ...
              <div class="financing-table" v-if="item.order" v-for="(order, index1) in item.order" :key='index1'>
                <div class="financing-table__heder">
                  ...
                  <div><span>订单金额（元）：</span><span>{{toThousands(order.amount)}}</span></div>
                  <!-- <div>
                    <span>电子原件或扫描件</span>
                  </div> -->
                </div>
                    ...
                    <div class="financing-table__tr" v-if='order,deposit' v-for="(deposit, index2) in order.deposit" :key='index2'>
                      <div class="financing-table-content">
                        <div>
                          ...
                          <div>
                            <span>票据金额（元）：</span>
                            <span>{{toThousands(deposit.amount)}}</span>
                          </div>
                          <div>
                            <span>汇票到期日：</span>
                            <span>{{deposit.payment_date}}</span>
                          </div>
                        </div>
                      </div>
                    </div>
...
export default {
  ...
  created () {
    this.$ajax.get(`/api/financing/application/${this.$route.query.financing_id}/`).then(res => {
      if (res.status === 200) {
        this.showList = res.data.application_data.chain_info
      }
    })
  },
  methods: {
    ...
    handleChain (el) {
      ...
      window.open(`#/fund/chain_info?id=${query.id}&financing_id=${query.financing_id}`)
    },
    // 点击企业名称跳转博客
    businessBlog (item) {
      const that = this
