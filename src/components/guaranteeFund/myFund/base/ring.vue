...
export default {
  ...
  created () {
    const that = this
    if (!that.cid) {
      this.$ajax.get(`/api/financing/application/${this.id}/`).then(res => {
        if (res.data.type === '1') {
          that.chainId = res.data.application_data.chain_id
        } else {
          if (res.data.application_data.loan_purpose.length !== 0) {
            that.chainId = res.data.application_data.loan_purpose[0].chain_id
          } else {
            that.chainId = res.data.application_data.first_repayment[0].chain_id
          }
        }
        that.$ajax.get('/api/chain/circle/contract_list/?chain_id=' + that.chainId).then(res1 => {
          ...
          that.$ajax.get(`/api/chain/circle/${res1.data.s_contract[0].id}/`).then(res2 => {
            ...
            res.data.status === '4' ? that.sPart.Repayment = [{}] : that.sPart.Repayment = []
            that.getting = true
          })
      ...
      that.$ajax.get('/api/chain/circle/contract_list/?chain_id=' + that.cid).then(res1 => {
        // 销售合同
        that.$ajax.get(`/api/chain/circle/${res1.data.s_contract[0].id}/`).then(res2 => {
          that.sPart = res2.data.info
          that.sPart.Contract = res1.data.s_contract
  ...
  methods: {
    ...
    hanldeDetail (item, key, sw) {
      ...
      let str = key !== 'Contract' ? urls[key] + item.id + '&chain_id=' + that.chainId : urls[key] + '&chain_id=' + that.chainId + '&barIndex=2'
