        ...
        <div class='slide_info'>
          ...
          <div>
            ...
            <div class='show_tab' v-if="showNumber === index.toString() + 6" style="left: -600px;">
              <firstCardR :id='item.financing_id' />
            </div>
          </div>
          <!-- 第一还款来源逾期率 -->
          <div>
...
<script>
import idCardR from '../table_card/id_card_r'
import usageCard from '../table_card/usage_card'
import firstCardR from '../table_card/first_card_r'
...
export default {
  name: 'application',
  components: {
    idCardR, usageCard, firstCardR, secondCard, customerCard, fundCard, businessCard, financingRing
  },
  data () {
    return {
      URL: this.$root_url(),
  ...
  methods: {
    handleAjax (page, status, ps) {
      ...
      // 融资列表
      that.$ajax.get('/api/financing/application/?type=0&' + 'page=' + p + '&page_size=' + pz + '&helping_status__in=' + status).then(res => {
        let showList = []
        res.data.results.forEach(val => {
          showList.push({
            ...
            // rate: val.repayment_rate ? val.repayment_rate + '%' : val.loan_info.expect_rate + '%'
          })
        })
