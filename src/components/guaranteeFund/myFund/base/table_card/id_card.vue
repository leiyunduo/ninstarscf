...
export default {
      ...
      if (res.status === 200) {
        ...
        if (!res.data.is_helping) {
          ...
          that.fund = {
            ...
            ...res.data.fund_info,
            show_rate: !res.data.helping_data.rate ? res.data.member_info.rate : res.data.helping_data.rate
          }
        }
        that.base = {
        ...
        that.pic = {
          ...
          usage: that.handleShotage(res.data.application_data.first_repayment) * 0.7,
          first_sum: that.handleSum(res.data.application_data.first_repayment),
          shortage: that.handleShotage(res.data.application_data.chain_info)
        }
        that.sum = {
          usage: that.handleSum(res.data.application_data.chain_info),
          first: that.handleSum(res.data.application_data.first_repayment),
          second: that.handleSum1(res.data.second_repayment),
          other: that.handleSum1(res.data.application_data.other_collateral)
        }
  ...
  methods: {
    ...
    handleOrder (item, r, tar) {
      ...
      window.open(`#/myFund/order_list?partner=${partner}&company=${company}&r=${tar}`)
