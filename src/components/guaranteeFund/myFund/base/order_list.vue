    ...
    <div class='order_list'>
      ...
      <div class="order_wrap">
        <div v-for='(item, index) in orderList' :key='index' class='chain_item'>
          ...
          <p class="item_title" v-if="item.s_data.length !== 0">
            <span>应收记账</span>
          </p>
          <ul class="item_list" v-if="item.s_data.length !== 0">
          ...
          <p class="item_title_b" v-if="item.p_data.length !== 0">
            <span>应付记账</span>
          </p>
          <ul class="item_list" v-if="item.p_data.length !== 0">
            <li class='order_title'>
              <div>采购订单</div>
              <div>所属合同</div>
              <div>订单创建时间</div>
...
export default {
  ...
  methods: {
    ...
    handleChain (item, el, tp) {
      ...
      // const that = this
      if (el === 1) {
        window.open(`#/credit_app/chain_detail/order?contract_id=${item.contract_id}&chain_id=${item.chain_id}&chian_type=${item.chain_type}&order_id=${item.id}&barIndex=2`)
      } else {
        window.open(`#/credit_app/chain_detail/contract?contract_id=${item.contract_id}&chain_id=${item.chain_id}&chian_type=${item.chain_type}&contract_type=${tp}&barIndex=2`)
      }
    }
  }
}
