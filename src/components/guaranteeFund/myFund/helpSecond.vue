      ...
      <ul class="cover_pool_list" v-if="showList.length !== 0">
        <li v-for="(item, index) in showList" :key='index'>
          ...
          <div class="show_item_infoline">
            <div>
              <span>销售项目：</span>
              <span style="margin-right: 12px;color:#eb6100;text-decoration:underline;cursor:pointer;" @click="handleChain(item)">{{item.contract_name}}</span>
            </div>
            <div>
              <span>付款人：</span>
              <span :style="!item.payment_info || !item.payment_info.payment  ? 'color: #999' : ''" @click="businessBlog(item)">{{item.payment_info ? (item.payment_info.payment || '未填写') : '未填写'}}</span>
            </div>
          </div>
          <div class="show_item_infoline">
            <div>
            ...
            <div>
              <span>票据金额（元）：</span>
              <span style="color: #333">{{toThousands(item.amount)}}</span>
            </div>
            <div>
              <span>汇票到期日：</span>
            ...
            <div class="div_list_content" v-if="otherList.length !== 0" v-for="(item, index) in otherList" :key="index">
              <div class="top">
                <ul>
                  ...
                  <li><span>市场估值（元）：</span><p style="font-weight: bolder">{{toThousands(item.appraisement)}}</p></li>
                  <li></li>
                </ul>
              </div>
              <div class="list" v-if="item.collateral_type === '1'">
                <ul>
                  ...
                  <li><span>面积（平米）：</span><p>{{item.area}}</p></li>
                  <li><span>评估报告：</span><p v-if="item.att_list.length === 0 || !item.att_list" style="color: #999">暂无报告</p><p class="color_onlike" v-if="item.att_list.length !== 0 && item.att_list" v-for="(it, index1) in item.att_list" :key='index1'>{{it.name}}</p></li>
                </ul>
              </div>
...
export default {
  ...
  data () {
    return {
      ...
      pics: [require('../../../../static/img/tickets/tq.png'), require('../../../../static/img/tickets/yq.png'), require('../../../../static/img/tickets/jq.png')]
    }
  },
  components: {
    // AppNav, 
  ...
  methods: {
    ...
    handleChain (el) {
      ...
      let str = ''
      el.chain_type === 2 ? str = '#/fund/chain_info?id=' : str = '#/fund/chain_info?id='
      window.open(str + query.id + '&financing_id=' + query.financing_id)
    },
    // 点击企业名称跳转博客
    businessBlog (item) {
      // const that = this
      // console.log(item)
      // window.open('#/enterpriseBlog?bid=' + item.partner_company + '&help=1' + '&financing_id=' + that.$route.query.financing_id)
    }
  }
}
</script>
