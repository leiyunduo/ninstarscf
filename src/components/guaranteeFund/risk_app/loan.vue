<template>
  <div class='loan_wrap'>
    <!-- <appNav :father="{selIndex: 3}" type='2'></appNav> -->
    <div class='loan_list'>
    ...
    <el-dialog
      ...
      <div class="financing_prosess">
        ...
        <ul v-if="showType === '0'">
          ...
          <li v-if="firstChain.first_repayment === 0">未提交自偿性还款来源监控</li>
        </ul>
      </div>
    </el-dialog>
    <el-dialog
...
import business from '@/components/guaranteeFund/myFund/base/table_card/business_card'
import idCard from '@/components/guaranteeFund/myFund/base/table_card/id_card'
import idCardR from '@/components/guaranteeFund/myFund/base/table_card/id_card_r'
...
export default {
  ...
  methods: {
    ...
    handleAjax (page, ps) {
      ...
      let p = page || 1
      let pz = ps || 6
      that.$ajax.get('/api/financing/application/?' + 'page=' + p + '&page_size=' + pz + '&status__in=3').then(res => {
        // console.log(res)
        if (res.status === 200) {
          let showList = []
