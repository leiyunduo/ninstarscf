<template>
  <div class="all">
    ...
    <div class="showData">
      <div class="content">
        <div class="title_data">资产合计</div>
        <AssetList :bid="company_id" :type="1" />
      </div>
      <!-- <div class="content">
        <div class="title_data title_data-line">单笔统计
          <el-dropdown id="dropdown" size="small" trigger="click" split-button type="primary" @command="handleCommand">
            {{contract_name}}
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item :command='item' v-for="(item, index) in selectList" :key="index">
                <span v-if="Number(item.type) === 2 && item.purchase_contract.length > 0">{{item.purchase_contract[0].name}}</span>
                <span v-if="Number(item.type) === 1 && item.sales_contract.length > 0">{{item.sales_contract[0].name}}</span>
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </div>
        <AssetList :bigData="bigData" :type="0" />
      </div> -->
    </div>
  </div>
</template>
