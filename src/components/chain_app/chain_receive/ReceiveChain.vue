<template>
  <div class="view_chain">
    ...
    <!-- <div class="chain_nav_top" :class="selIndex === 0 ? 'max-topHeight' : ''" v-show="element">
      <div class="topLocation">
        <location :location="location"></location>
      </div>
      <ring :chainId="chainId" v-if="selIndex === 0"/>
    </div> -->
    <div class="line_detail">
      ...
      <ul class="view_nav" :style="selIndex === 0 ? 'top: 110px;' : ''">
      ...
      <div v-if="selIndex === 0" class="chain_list" style="margin-top: 120px">
        ...
        <div class="chain-box">
          ...
          <div class="chain-btn-confirm" v-if="chain_identify">
            <div @click="sureConfirm()" :class="Number(chainStatus) !== 1 ? '' : 'disabled'">确认并提交平台</div>
            <div @click="cooperation(chainId)" :class="Number(chainStatus) === 0 ? 'disabled' : ''">合作企业确认</div>
          </div>
      ...
      <div v-if="selIndex === 2" class="chain_list">
        ...
        <data-analyse :isChain="true" :chainId="chainId" v-if='dataTime === 0' />
...
export default {
  ...
  methods: {
    chainIdentify (id) {
      this.$ajax.get(`/api/chain/${id}/identify/`).then(res => {
        this.chain_identify = res.data
      })
    },
    toInvite () {
    ...
    getChainStatus (chainId) {
      this.$ajax.get(`/api/chain/${chainId}/status/`).then(res => {
        ...
        // this.$store.commit('chain/setConfirm', false) //用于全局更新最新状态
      }).catch((err)=>{
    ...
    sureConfirm () {
      this.$confirm('此操作将向平台提交数据?', '提示', {
        ...
        this.$ajax.patch(`/api/chain/${this.$route.query.id}/verified/`).then(res=>{
          if (res.status === 200) {
            this.$message.success('操作成功!')
            // this.$store.commit('chain/setConfirm', true) //用于全局更新最新状态
          }
  ...
  mounted: function () {
    ...
    that.chainIdentify(that.chainId)
  }
}
</script>
