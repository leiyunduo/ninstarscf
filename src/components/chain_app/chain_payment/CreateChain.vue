  ...
  <div class="c_container">
    ...
    <div class="line_detail">
      ...
      <div class="line_title">
        <span>{{nameTitle[0]}}</span>
      </div>
      <div class="line_item" v-for='(title, index) in getting_titleList' :key='title.index' v-if='index <= 2'>
      ...
      <div class="line_title">
        <span>{{nameTitle[1]}}</span>
      </div>
      <div class="line_item inline" v-for='(title, index) in paying_titleList' :key='index'>
        <div class="item_symbol">
          <div></div>
        </div>
        <div class="item_info">{{title.name}}</div>
      ...
      <div class="line_title">
        <span>{{nameTitle[2]}}</span>
      </div>
      <div class="line_item" v-for='(title, index1) in getting_titleList' :key='title.index' v-if='index1 > 2'>
        <div class="item_symbol">
          <div></div>
        </div>
        <div class="item_info">{{title.name}}</div>
        <div class="create_btn" @click="create($event, 1)">{{title.operate}}</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      nameTitle: ['销售合同登记', '采购合同登记记账', '关联销售登记'],
      father: {
        selIndex: 1
      },
      getting_titleList: [
