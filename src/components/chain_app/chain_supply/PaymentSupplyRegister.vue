<template>
  <div class="role-person__payment">
    ...
    <div class="chain">
      <div class="card_list" v-if="!down1">
        <div class="card_item" v-for="(title, index) in titleList" :key="index" v-if="index <= 6 && sContract[index]">
              ...
              <div class="item_table" v-for="(one, trIndex) in sContract[index]" :key="trIndex">
                <div v-for="(log, logIndex) in one" :key="logIndex">
                  ...
                  <div class="table_line">
                    ...
                    <div>{{log.verifier_name ? log.verifier_name : '未审核'}}</div>
                    <div>{{log.approved_name ? log.approved_name : '未确认'}}</div>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="item_table" v-for="(one, trIndex) in sContract[index]" :key="trIndex">
                <div v-for="(log, logIndex) in one" :key="logIndex">
                  ...
                  <div class="table_line">
                    ...
                    <div>{{log.verifier_name ? log.verifier_name : '未审核'}}</div>
                    <div>{{log.approved_name ? log.approved_name : '未确认'}}</div>
                    <div class="td_loginfo">{{log.name}}</div>
                  </div>
                </div>
              </div>
            </div>
    ...
    data () {
      return {
        roleList: ['操作方', '审核方', '确认方', '服务方', '认证方', '经办人'],
          logList: ['变更内容', '操作方', '审核方', '确认方'],
          titleList: [
            {name: 'B2B销售合同', operate: '+ 添加B2B销售合同', null: '/receivableContractCreate'},
            {name: 'B2B订金', operate: '+ 添加B2B应收订金', null: '/receivableDepositCreate'},
            {name: 'B2B销售订单计划', operate: '+ 添加B2B销售订单计划', null: '/receivablePlanCreate'},
            {name: '仓储合同', operate: '+ 添加仓储合同', null: '/warehousingContract1'},
            {name: '运输合同', operate: '+ 添加运输合同', null: '/logisticsContract1'},
            {name: '参与角色登记', operate: '+ 添加参与角色', null: '/roleRegistration'},
            {name: '融资计划', operate: '+ 添加融资计划', null: '/financingPlan1'},
            {name: 'B2B电子订单', operate: '+ 添加B2B电子订单', null: '/orderList1'},
            {name: 'B2B电子出货单', operate: '+ 添加B2B电子出货单', null: '/waybillList1'},
            {name: 'B2B电子运单', operate: '+ 添加B2B电子运单', null: '/receipt1'},
            {name: 'B2B验收入库单', operate: '+ 添加B2B验收入库单', null: '/WarehouseReceiptList1'},
            {name: 'B2B订单结算', operate: '+ 添加B2B订单结算', null: '/settlement1'},
            {name: 'B2B发票', operate: '+ 添加B2B发票', null: '/invoice1'},
            {name: 'B2B支付', operate: '+ 添加B2B支付', null: '/payment1'},
            {name: 'B2B合同结算', operate: '+ 添加B2B合同结算', null: '/contractSettlement1'}
          ],
          relatedTableList: [
    ...
    mounted () { 
      ...
      this.$ajax.get(`/api/chain/five_stream/${that.$route.query.id}/ledger_log/`).then(res => {
        if (res.status === 200) {
          ...
          this.sContract = this.filterSupply(this.filterObj(res.data.s_contract))
          this.pContract = this.filterArray(res.data.p_contract)
          this.pContract.length !== 0 ? this.selContract = this.filterSupply(this.pContract[this.selIndex]) : this.selContract = []
        }
      })
    },
    methods: {
      changeTab (index) {
        this.selContract = this.filterSupply(this.pContract[index])
        this.selIndex = index
      },
