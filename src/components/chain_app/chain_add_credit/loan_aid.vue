    ...
    <div class="flex-add">
      <div class="loan_aid">
        <div class="list">
          ...
          <!-- 风控大厅 -->
          <div class="echarts_list" style="page-break-after:always;">
            <div class="list_nav">
              <h3>风控大厅</h3>
            </div>
            <div class="list_data" v-for="(item, index) in financing_purposes" :key="index" v-if="financing_purposes.length > 0">
              <div class="list_data-list">
                ...
                <div class="content_list" style="height:300px;">
                  <h3>融资用途监控</h3>
                  <div class="data_list">
            ...
            <div class="list_data">
              <div class="list_data-list">
                <div class="top_list">
                  ...
                  <ul style="border-bottom:1px solid rgba(221,221,221,1);padding-bottom:10px;" class="ul">
                    <li><p>质押池估值（元）：</p><span>{{toThousands(g_p_list.p_valuation)}}</span></li>
                    <li><p>担保池估值（元）：</p><span>{{toThousands(g_p_list.g_valuation)}}</span></li>
                  </ul>
...
export default {
  ...
  mounted () {
    ...
    // 融资用途监控
    that.$ajax.get('/api/financing/application/?status__in=3' + '&company_id=' + that.company_id).then(res => {
      if (res.status === 200) {
