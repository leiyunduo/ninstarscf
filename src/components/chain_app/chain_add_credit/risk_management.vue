      ...
      <div class="riskManagement">
        <div class="list">
          ...
          <div class="echarts_list">
            ...
            <div class="list_data">
              <div id="call_the_police" style="height:400px;" v-if='Number(prints) !== 1'></div>
              <img :src="call_the_police_img" alt="" v-if='Number(prints) === 1'>
            </div>
          </div>
        </div>
        <div class="list">
              ...
              <div class="business_grade1">
                ...
                <div class="right_left_list">
                  ...
                  <div class="left_right">
                    钒作为冶炼合金钢的重要原料，可提高钢的强度、延展性和韧性。钒合金广泛用于交通
                    运输、机械制造、建筑、航天航空等。
                  </div>
                </div>
                <div class="right_left_list">
                  ...
                  <div class="left_right">
                    目前我国钒制品的生产企业已达50余家，未来十年钒产品的消费需求仍会持续增
                    加，中型与特色钒企业将批量涌现，业内竞争更加激烈，行业性市场过剩局面在未来
                    几年仍有持续加剧之势，钒产品价格在稳定中保持下滑的趋势近期不会改变。
                  </div>
                </div>
                <div class="right_left_list">
                  <div class="right_left">行业技术风险：</div>
                  <div class="left_right">
                    虽然我国是世界第二大钒产品生产基地，但一些关键技术瓶颈尚未突破，高端钒产
                    品及相关新工艺的开发能力仍然不足，产品的核心竞争力不强。此外，不同钒氮合金生
                    产企业的生产工艺虽然大同小异，但生产过程中原料配比、温度等各项指标对钒氮合金
                    转化率、质量的影响较大。
                  </div>
                </div>
                <div class="right_left_list">
                  ...
                  <div class="left_right">
                    供货量不足、原材料涨价或者质量问题等不可测因素，影响钒氮合金的产量、生
...
export default {
  ...
  mounted () {
    ...
    that.pieChart('call_the_police', that.call_the_police.legend, that.call_the_police.titles, that.call_the_police.data, that.call_the_police.color)
  },
  watch: {
    'prints' (n, o) {
      const that = this
  ...
  methods: {
      ...
      var labelOption = {
        normal: {
          ...
          formatter: function (i) {
            let str;
            if (i.seriesName === '流动资金需求情况') {
              str = that.toThousands(i.value) + '（元）'
              str += '  {per|' + i.percent + '%}  '
            } else if (i.seriesName === '订单情况（年统计）') {
              str = i.value
              str += '  {per|' + i.percent + '%}  '
            } else if (i.seriesName === '交易逾期情况') {
              str = i.value
              str += '  {per|' + i.percent + '%}  '
            } else if (i.seriesName === '应收账款/已收账款') {
              str = that.toThousands(i.value) + '（元）'
              str += '  {per|' + i.percent + '%}  '
            } else if (i.seriesName === '应付账款/已付账款') {
              str = that.toThousands(i.value) + '（元）'
              str += '  {per|' + i.percent + '%}  '
            }
            return str;
    ...
    columnImg () {
      const that = this
      // 2017
      let a = echarts.init(document.getElementById('call_the_police'))
      var a1 = a.getDataURL()
      that.call_the_police_img = a1
    }
  }
}
</script>
