        ...
        <div class="chain-order__form">
          <div class="chain-order__form__group chain_flex chain-width-r7">
            <div class="chain-order__form__row">
              ...
              <input type="text" :disabled="btnList.length > 1 ? true : false" v-model="order.orderName">
            </div>
            <div class="chain-order__form__row">
              <span class="chain-required-orange">*</span>
            ...
            <div class="chain-order__form__row ">
              ...
              <el-date-picker size="small" type="date" :disabled="isDisabled.orderTransportDate" v-model="order.orderTransportDate"  placeholder="选择日期" format="yyyy-MM-dd" value-format="yyyy-MM-dd"></el-date-picker>
            </div>
            <div class="chain-order__form__row ">
              <span class="chain-required-orange">*</span>
              <label for="">入库日期:</label>
              <el-date-picker size="small" type="date" :disabled="isDisabled.orderAccptanceDate" v-model="order.orderAccptanceDate" placeholder="选择日期" format="yyyy-MM-dd" value-format="yyyy-MM-dd"></el-date-picker>
            </div>
            <div class="chain-order__form__row">
              <span class="chain-required-orange">*</span>
...
export default {
  name: 'chainorderEdit',
  components: {
    ChainTableEdit, ChainUploadEdit, ChainUploadView, ChainTableView, navBar,
    Agent, AgentInviteEdit, AgentInviteView
  },
  props: ['orderId', 'chainId', 'chainType', 'type'],
  data () {
    return {
      step: {selIndex: 1},
      order: {
      ...
      orderErrMsg: {
        ...
        orderDriverDate: '出库日期不能为空',
        orderTransportDate: '运输日期不能为空',
        orderAccptanceDate: '入库日期不能为空',
      ...
      isDisabled: {
        ...
        orderAccptanceDate: false,
        orderTransportDate: false,
  ...
  methods: {
    ...
    sureCommit () {
      ...
      let params = {
        ...
        name: this.btnList.length > 1 ? this.btnList[0].name + `(变更${this.btnList.length -1})` : this.order.orderName,
        date_created: this.order.orderFinishDate,
        deliver_date: this.order.orderDriverDate,
        transport_date: this.order.orderTransportDate,
        acceptance_date: this.order.orderAccptanceDate,
        settlement_date: this.order.orderSettleDate,
        amount: this.order.orderValue,
        note: this.order.orderDesc,
        agent: _agent
      }
      this.orderEdit(this.orderId, params)
    },
    orderEdit(order_id, params) {
      this.$ajax.patch(`/api/chain/order/${order_id}/`, params).then(res => {
        if (res.status === 200) {
