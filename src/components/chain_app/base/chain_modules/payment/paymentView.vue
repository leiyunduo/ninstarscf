<template>
  <div class="settlement_edit">
    ...
    <div class="form">
      ...
      <div class="form_line" v-if="subData.type === 0">
        <div class="form_section">
          <div class="form_label"><span class="required">*</span>支付金额</div>
          <div class="item_info">
      ...
      <div class="form_line" v-if="subData.type === 1">
        ...
        <div class="form_section">
          <div class="form_label"><span class="required">*</span>收款企业</div>
          <div class="item_info">
            <div :class="!subData.gathering ? 'is_null' : ''">{{subData.gathering || '未填写'}}</div>
          </div>
        </div>
        <div class="form_section">
          <div class="form_label"><span class="required">*</span>付款企业</div>
          <div class="item_info">
        ...
        <div class="form_section">
          <div class="form_label"><span class="required">*</span>收款机构</div>
          <div class="item_info">
            <div :class="!subData.gatheringCompany ? 'is_null' : ''">{{subData.gatheringCompany || '为填写'}}</div>
          </div>
        </div>
        <div class="form_section">
          <div class="form_label"><span class="required">*</span>付款机构</div>
          <div class="item_info">
      ...
      <div class="form_line" v-if="subData.type > 1">
        ...
        <div class="form_section">
          ...
          <div class="item_info">
            <div :class="!subData.ticket_date ? 'is_null' : ''">{{subData.ticket_date || '未填写'}}</div>
          </div>
        </div>
        <div class="form_section">
    ...
    handleAjax (id, sw) {
      ...
      that.$ajax.get(`/api/chain/payment/${id}/`).then(res => {
        if (res.status === 200) {
          // 前后台不统一
          res.data.type -= 1
          that.subData = { ...res.data, ...res.data.payment_info, agent: res.data.agent_list }
          that.uploadData = res.data.att_list
          if (sw) {
            let slide = []
            res.data.id_list.forEach((val, i) => {
              i === 0 ? slide.push({name: val.name, id: val.id}) : slide.push({name: '变更' + i, id: val.id})
            })
            this.$emit('childInfo', {slide, status: res.data.status, contractId: res.data.contract_id})
          }
          sessionStorage.originSettlementId = that.subData.settlement_id //将改支付所属的结算ID存入session中，以便后续变更时备用
        }
      })
    }
  },
