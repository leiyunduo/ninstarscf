    ...
    <div class="chain-contract__item">
      ...
      <div class="chain-contract__body">
        ...
        <div class="chain-contract__content">
          ...
          <p>第二条：保管期间经双方约定自
            <el-date-picker  type="daterange" v-model="contract.contract_time" :disabled="isDisabled.contract_time" size="mini" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" format="yyyy - MM - dd 日" value-format="yyyy-MM-dd">
            </el-date-picker>
            止。
          </p>
...
export default {
  ...
  props: ['chainId', 'warehouseId', 'contractId', 'chainType'],
  data () {
    return {
      disabled: true,
      step: {selIndex: 1},
  ...
  methods: {
    ...
    contractPatch: function(prev_id, params) {
      ...
      this.$ajax.post('/api/chain/warehouse_contract/', params).then(res => {
        if (res.status === 201) {
