      ...
      <div class="outbound_list">
        ...
        <div class="list">
          <div v-if="showIndex === buttonList.length-1 && isAdding">
            <waybill-Edit :create='create' :cid="cid" :ruleFormList='ruleFormList' :chainId="chain_id" :chainType="chain_type" :orderId="order_id"/>
          </div>
          <div v-else>
...
export default {
  ...
  components: {
    Location, 
    // AppNav, 
    waybillEdit, waybillList, navBar
  },
  created () {
    var that = this
    ...
    that.$ajax.get(`/api/chain/transport/${that.id}`).then(res => {
      if (res.status === 200) {
        ...
        that.cid = that.$route.query.contract_id || res.data.contract_id
