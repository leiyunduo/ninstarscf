    ...
    <div class="invoice_section">
      ...
      <div class="order_nav">
        <ul>
          ...
          <li v-if="btnStatus === '1' && showEdit" @click="handleSupply">
            <span>+添加变更</span>
          </li>
        </ul>
      </div>
...
export default {
  ...
  created () {
    ...
    this.company_id = localStorage.id
    this.location = [
      {title: '链式记账', url: url},
      {title: '发票'}
    ]
    if (that.$route.query.invoice_id) {
      that.$ajax.get(`/api/chain/invoice/${that.$route.query.invoice_id}/`).then(res => {
        ...
        res.data.id_list.forEach((val, i) => {
          ...
          i === 0 ? org.push({id: val.id, name: val.name}) : org.push({id: val.id, name: '变更' + i})
        })
        that.slideNav = slide
        that.slideOrg = org
  ...
  methods: {
    ...
    handleEdit () {
      ...
      if (that.showEdit) {
        that.showEdit = false
        that.showItem = {subData: that.$children[1].subData, uploadData: that.$children[1].uploadData}
      } else {
        that.handleSubmit()
      }
    },
