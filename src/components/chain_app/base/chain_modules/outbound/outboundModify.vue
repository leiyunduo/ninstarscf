  ...
  <div id="outboundModify">
    ...
    <div class="outbound">
      ...
      <div class="outbound_list">
        ...
        <div class="list">
          <div v-if="showIndex === buttonList.length-1 && isAdding">
            <outbound-Edit :create='create' :cid="contract_id" :ruleFormList='ruleFormList' :chainId="chain_id" :chainType="chain_type" :orderId="order_id"/>
          </div>
          <div v-else>
...
export default {
  ...
  components: {
    Location, 
    // AppNav, 
    outboundEdit, navBar, outboundList
  },
  created () {
    var that = this
    ...
    that.$ajax.get(`/api/chain/warehouse/${that.id}`).then(res => {
      if (res.status === 200) {
        ...
        that.ruleFormList.storage_list = [] //TODO
        ...
        that.contract_id = that.$route.query.contract_id || res.data.contract_id
        that.order_id = that.$route.query.order_id || res.data.order_id
        let arr = []
