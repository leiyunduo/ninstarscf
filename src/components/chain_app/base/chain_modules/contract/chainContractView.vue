  ...
  <div class="chain-contract">
    <div class="chain-contract__box">
      ...
      <div class="chain-contract__box__wrap">
        ...
        <div v-if="navIndex === 0">
          <chain-contract-create :contractType="contract_type" :chainId="chain_id" :chainType="chain_type"/>
        </div>
        <div v-else-if="navIndex === 1">
          <chain-contract-edit :chainId="chain_id" :contractId="contract_id" :chainType="chain_type" :contractType="contract_type"/>
        </div>
        ...
        <div v-else>
          <chain-contract-info @toEdit='changeView' :contractId="contract_id"/>
        </div>
      </div>
    </div>
  </div>
...
import chainContractEdit from './chainContractEdit'
import chainContractModify from './chainContractModify'
import chainContractCreate from './chainContractCreate'
import chainContractInfo from './chainContractInfo'
export default {
  ...
  components: {
    // AppNav, 
    Location, chainContractEdit, chainContractInfo, chainContractModify, chainContractCreate
  },
  mounted () {
  ...
  beforeCreate () {
    ...
    let _url = "", _title = ''
    if (Number(this.contract_type) === 1) {
      _title = '销售项目'
    } else if (Number(this.contract_type) === 2) {
      _title = '采购项目'
    } else {
      _title = '协同确认'
    }
    ...
    if (this.$route.query.claim) {
      ...
      this.location = [
        {
          title: this.contract_type ? '链式记账' : '协同展示',
          url: _url
        },
        {
          title: _title
