      ...
      <happy-scroll resize size="6">
        <div class="section_title">
          <ul class="tab-menu">
            ...
            <li @click="handleCreate(0)" class="tab-menu__item" :style="(showItem.length === 0 ||showItem.length === 1) ? '' : 'margin-left: -2px'" :class="style.tab.add"><span class="tab-menu__item__icon"></span><span>添加订单</span></li>
          </ul>
        </div>
      </happy-scroll>
    </div>
    <div class="tab-menu-group" v-if="hasContract && type === '2'">
      <happy-scroll resize size="6">
        <div class="section_title">
          <ul class="tab-menu">
            <li class="tab-menu__item" :class="setIndex===index ? style.tab.active:''" @click="changeTab(index, item)" v-for="(item, index) in showItem" :key = index>{{item.name}}</li>
        ...
        <div v-if="index === 0" class="show_line">
              ...
              <el-tooltip placement="top" v-if="showList[0][0].acceptance_overtime && 
                ...
                <div slot="content">
                  <p class="orderTitle" v-if="showList[0][0].acceptance_overtime.warehouse || showList[0][0].acceptance_overtime.warehouse === 0">
                    <span v-if="showList[0][0].acceptance_overtime.warehouse >= 0">距该订单出货日期还剩<strong>{{showList[0][0].acceptance_overtime.warehouse}}</strong>天，请及时记录相关<strong class="text">出货单</strong></span>
                    <span v-else>该订单出库<strong class="text">已逾期{{Math.abs(showList[0][0].acceptance_overtime.warehouse)}}</strong>天，请尽快记录相关<strong class="text">出货单</strong></span>
                  </p>
                  <p class="orderTitle" v-if="showList[0][0].acceptance_overtime.transport || showList[0][0].acceptance_overtime.transport === 0">
                    <span v-if="showList[0][0].acceptance_overtime.transport >= 0">距该订单运输日期还剩<strong>{{showList[0][0].acceptance_overtime.transport}}</strong>天，请及时记录相关<strong class="text">运单</strong></span>
                    <span v-else>该订单运输<strong class="text">已逾期{{Math.abs(showList[0][0].acceptance_overtime.transport)}}</strong>天，请尽快记录相关<strong class="text">运单</strong></span>
                  </p>
          ...
          <div class="show_line_detail" v-else-if="orderwarn || orderwarn === 0">
            <div class="chain-flex is_null">
              ...
              <span style="margin-left: 6px">暂未登记订单</span>
            </div>
          </div>
...
export default {
  ...
  mounted () {
    ...
    if(this.type === '1') {
      ...
      this.id ? this.hasContract = true : this.hasContract = false
    }
    if(this.id) {
