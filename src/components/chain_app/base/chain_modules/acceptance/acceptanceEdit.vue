  ...
  <div id="acceptanceEdit">
    <div class="top">
      ...
      <div class="top_bottom">
        ...
        <div class="right">
          <p><span>*</span>出单时间</p>
          <el-date-picker size="mini" v-model="ruleForm.date_created" type="date" placeholder="选择日期" format="yyyy 年 MM 月 dd 日" value-format="yyyy-MM-dd"></el-date-picker>
        </div>
      </div>
    </div>
    <div class="center_person">
      <h3>入库管理员登记</h3>
      <ul class="ul-list">
        <li><small class="required">*</small><span class="_title">姓名</span><input type="text" maxlength="32" v-model="ruleForm.keeper" placeholder="请输入姓名"></li>
        <li><small class="required">*</small><span class="_title">手机号</span><input type="text" v-model="ruleForm.phone" maxlength="11" placeholder="请输入手机号"></li>
        <li v-if="create === 1 || (create === 0 && isOrigin)">
          <span class="_title">公司名称</span><input type="text" placeholder="自储" maxlength="32" @keyup="getCompanyList($event, ruleForm.service_company_name, 3)" v-model="ruleForm.service_company_name">
        ...
        <li v-else>
          <span class="_title">公司名称</span><input type="text" maxlength="32" placeholder="自储" disabled v-model="ruleForm.service_company_name">
        </li>   
  ...
  methods: {
    ...
    saveDraft () {
      ...
      if (that.ruleForm.name === '') {
        that.$message.error('请输入入库单标题')
      } else {
        if (!that.ruleForm.date_created) {
          that.$message.error('请输入出单时间')
        } else {
                ...
                if (!/^[0-9]+(.[0-9]{2})?$/.test(that.ruleForm.amount)) {
                  ...
                  if (that.create === 1) {
                    ...
                    if (that.ruleForm.service_company_name && !that.ruleForm.service_company_id) {
                      that.createFakeCompany(that.ruleForm.service_company_name, 3, _params).then(this.acceptCreate).catch((reason)=>{
                        console.err(reason)
                      })
                    } else if (!that.ruleForm.service_company_name) {
                      _params.service_company_id = null
                      that.acceptCreate(_params)
                    } else {
                      that.acceptCreate(_params)
                    }
                    ...
                    if (that.ruleForm.service_company_name && that.ruleForm.service_company_name != '自储' && !that.ruleForm.service_company_id) {
                      that.createFakeCompany(that.ruleForm.service_company_name, 3, _params).then(this.acceptUpdate).catch((reason)=>{
                        console.error(reason)
                      })
                    } else if (!that.ruleForm.service_company_name) {
                      _params.service_company_id = null
                      that.acceptUpdate(_params)
                    } else {
                      that.acceptUpdate(_params)
                    }
                  } else if (that.create === 0 && !this.isOrigin) {
    ...
    acceptCreate: function(params) {
      this.$ajax.post('/api/chain/acceptance/', params).then(res => {
        if (res.status === 201) {
    ...
    acceptUpdate (params) {
      ...
      this.$ajax.patch(`/api/chain/acceptance/${this.id}/`, params).then(res => {   
        if (res.status === 200) {
          let _url = (this.chainType === `1`) ? '/chain_app/receive_chain?id='+this.chainId : '/chain_app/payment_chain?id='+this.chainId
          this.$router.push({
            path: _url
...
#acceptanceEdit, #acceptanceOrder, #acceptanceModify {
