<template>
  <div id="acceptanceList">
    <div class="top">
      <div class="top_top">
        <h3>验收入库单<p>创建时间：<span>{{ruleForm.create_time ? ruleForm.create_time.replace('T', ' ').substring(0, ruleForm.create_time.indexOf('.')) : ''}}</span></p></h3>
      </div>
    ...
    <div class="center">
      ...
      <div class="product">
        ...
        <div class="chain_flex_end">
          <span class="chain-text">参考合计：<strong class="isDisabled">{{toThousands(ruleForm.total_val) || ''}}</strong>(系统统计合计，仅限于参考)</span>
        </div>
      </div>
    </div>
    ...
    <div class="center_remark">
      <h3>添加备注及说明</h3>
      <div>
        <textarea disabled v-model="ruleForm.note"></textarea>
      </div>
    </div>
  </div>
</template>
