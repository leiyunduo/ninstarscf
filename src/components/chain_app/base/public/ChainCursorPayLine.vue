    ...
    <div class="inner">
      <div id="info" v-show="showInfo">
        <div class="inline_box" id="box1" :style="style.mainLine.showData.border1">
          ...
          <div class="inline_item" :style="style.mainLine.related.item_c">
          </div>
        ...
        <div class="inline_box" id="box2" :style="style.mainLine.related.border1">
          <div class="inline_item" :style="style.mainLine.related.item1">
          </div>
          <div class="inline_item_text" @click="lineHandler($event)">
            <span>采购登记</span>
          </div>
          <div class="placeholder" v-show="placeholder2" :style="holder"></div>
        </div>
        <div class="inline_box" id="box3" :style="style.mainLine.related.border2">
          <div class="inline_item" :style="style.mainLine.related.item_c1">
          </div>
          <div class="inline_item_text" @click="lineHandler($event)">
            <span>应付记账</span>
          </div>
        ...
        <div class="inline_box" id="box4" :style="style.mainLine.showData.border2">
      ...
      <div id="detail" v-show="!showInfo">
        <div class="inline_box_detail" v-if="flag === 1 || flag === 2" :style="style.inline_item_width.min" :class="flag===1?'lineColorOrange':(isHavingContract(showData, 0, 8) ? 'lineColorOrange':'')">
          ...
          <div v-else class="inline_item" :class="flag === 1 ? 'border_o':''" :style="flag === 2? 'width:26px;height:26px;background-image: url(static/img/cursor/icon4.png);top:-13px':checkStatus(dataList[0])">
          ...
          <div class="inline_item_text" id="box2">
            ...
            <div class="placeholder11" v-if="flag === 2" style="right: 14px"></div>
        ...
        <div class="inline_box_detail" :class="dataList[index].length > 0 ? 'lineColorOrange' : ''" v-if="flag === 1 && index !== 0" v-for="(val, index) in titleList" :key="index" :style="titleList.length === 8 ? style.inline_item_width.mid : style.inline_item_width.max">
          ...
          <div v-else class="inline_item" :class="checkStatus(dataList[index]) === '' ? 'border_o':''" :style="dataList[index].length === 0 ? style.inline_item: checkStatus(dataList[index])" :id="index" @click="toAnchor($event)">
        ...
        <div class="inline_box_detail" :class="dataList[index].length > 0 ? 'lineColorBlue' : ''" v-if="flag === 2" v-for="(val, index) in titleList" :key="index" :style="titleList.length === 8 ? style.inline_item_width.mid : style.inline_item_width.max">
          <div v-if="index === placeHolder[0]" class="twinkle" :id="'_' + index" @click="toAnchor($event)">
          </div>
          <div v-else class="inline_item" :class="checkStatus(dataList[index]) === '' ? 'border_b':''" :style="dataList[index].length === 0 ? style.inline_item: checkStatus(dataList[index])" :id="'_' + index" @click="toAnchor($event)">
        ...
        <div class="inline_box_detail" :class="[isHavingContract(related, 0, 8) ? 'lineColorBlue' : '']" v-if="flag === 1" :style="style.inline_item_width.min">
        ...
        <div class="inline_box_detail" :class="[isHavingContract(related, 7) ? 'lineColorBlue' : '']" v-if="flag === 1" :style="style.inline_item_width.min">
          <div class="inline_item" @click="lineHandler($event)" style="width:26px;height:26px;background-image: url('static/img/cursor/icon4.png');top:-13px">
          </div>
        ...
        <div class="inline_box_detail" :class="[isHavingContract(showData, 7) ? 'lineColorOrange' : '']" v-if="flag === 1" :style="style.inline_item_width.min">
          <div class="inline_item" style="width:26px;height:26px;background-image: url('static/img/cursor/icon3.png');top:-13px" @click="lineHandler($event)">
          </div>
        ...
        <div class="inline_box_detail" :class="[isHavingContract(showData, 7) ? 'lineColorOrange' : '']" v-if="flag === 2" :style="style.inline_item_width.min">
          <div class="inline_item" style="width:26px;height:26px;background-image: url('static/img/cursor/icon3.png');top:-13px" @click="lineHandler($event)">
          </div>
          <div class="inline_item_text" id="box4">
            <span class="font_size_f" @click="lineHandler($event)">
        ...
        <div class="inline_box_detail" :class="[isHavingContract(related, 0, 8) ? 'lineColorBlue' : '']"  v-if="flag === 3" :style="style.inline_item_width.min">
          <div class="inline_item" style="width:26px;height:26px;background-image: url('static/img/cursor/icon4.png');top:-13px" @click="lineHandler($event)">
          </div>
          <div class="inline_item_text" id="box3">
            <span class="font_size_f" @click="lineHandler($event)">
        ...
        <div class="inline_box_detail" :class="[dataList[index].length > 0 ? 'lineColorBlue' : '']" v-if="flag === 3" v-for="(val, index) in titleList" :key="index" :style="titleList.length === 8 ? style.inline_item_width.mid : style.inline_item_width.max">
          ...
          <div v-else class="inline_item" :class="checkStatus(dataList[index]) === '' ? 'border_b':''" :style="dataList[index].length === 0 ? style.inline_item: checkStatus(dataList[index])" :id="index+7" @click="toAnchor($event)">
          ...
          <div class="inline_item_text">
            <span :id="'_'+(index+7)" @click="toAnchor($event)" :class="(index + 7) === setIndex ? 'active_b':''">
              {{val.name}}
            </span>
          </div>
        </div>
        <div class="inline_box_detail" :class="[isHavingContract(showData, 7) > 0 ? 'lineColorOrange' : '']" v-if="flag === 3" :style="style.inline_item_width.min">
          <div class="inline_item" style="width:26px;height:26px;background-image: url('static/img/cursor/icon3.png');top:-13px" @click="lineHandler($event)">
          </div>
          <div class="inline_item_text" id="box4">
            <span class="font_size_f" @click="lineHandler($event)">
        ...
        <div class="inline_box_detail" :class="[isHavingContract(related, 0, 8) ? 'lineColorBlue' : '']" v-if="flag === 4" :style="style.inline_item_width.min">
          ...
          <div id="box3" class="inline_item_text">
            <span class="font_size_f" @click="lineHandler($event)">
              应付记账
            </span>
          </div>
				</div>
        <div class="inline_box_detail"  :class="[isHavingContract(related, 7) ? 'lineColorBlue' : '']" v-if="flag === 4" :style="style.inline_item_width.min">
          <div class="inline_item" style="width:26px;height:26px;background-image: url('static/img/cursor/icon3.png');top:-13px" @click="lineHandler($event)">
          </div>
          <div class="inline_item_text" id="box4">
            <span class="font_size_f" @click="lineHandler($event)">
            ...
            <div class="placeholder" style="right: 14px"></div>
          </div>
				</div>
        <div class="inline_box_detail" :class="[dataList[index].length > 0 ? 'lineColorOrange' : '']" v-if="flag === 4" v-for="(val, index) in titleList" :key="index" :style="titleList.length === 8 ? style.inline_item_width.mid : style.inline_item_width.max">
          <div v-if="index === placeHolder[0]" class="twinkle" :id="'_'+(index+7)" @click="toAnchor($event)">
          </div>
          <div v-else class="inline_item" :class="checkStatus(dataList[index]) === '' ? 'border_o':''" :style="dataList[index].length === 0 ? style.inline_item: checkStatus(dataList[index])" :id="'_'+(index+7)" @click="toAnchor($event)">
            <span class="inline_item_num" style="color: #ea6100" :id="'_'+(index+7)" v-if="!checkStatus(dataList[index])">
              <!-- {{dataList[index].length || ''}} -->
            </span>
            <span v-if="dataList[index].length > 0" :class="dataList[index].length > 4 ? 'tooltiptext_bottom':'tooltiptext_top'">
							<a v-for="(val,index1) in dataList[index]" :key="index1">
...
export default {
  ...
  methods: {
    lineHandler (e, type) {
      ...
      var data = {}
      this.setIndex = ''
      if (!type) that.showInfo = !that.showInfo
      switch (tag) {
        case 'box1':
    ...
    update () {
      ...
      that.$ajax(op).then(res => {
        ...
        if (res.data.receivable_contracts.length !== 0) {
          ...
          showData = [[{name: '', status: ''}], [], [], [], [], [], [], [], [], [], [], [], [], [], []]
        }
        that.showData = showData
        if (that.related[0].length > 0) {
          that.style.mainLine.related.item1 = 'width:26px;height:26px;background-image:url("static/img/cursor/icon4.png");top:-13px'
          that.style.mainLine.related.border1 = 'border-color: #0c8dff'
          that.style.mainLine.related.item_c = 'width:56px;height:26px;border-radius:0;background-image:url("static/img/cursor/icon_con1.png");top:-13px'
          if (that.isHavingContract(that.related, 7)) {
            that.style.mainLine.related.item2 = 'width:26px;height:26px;background-image:url("static/img/cursor/icon4.png");top:-13px'
            that.style.mainLine.related.border2 = 'border-color: #0c8dff'
            that.style.mainLine.related.item_c1 = 'width:56px;height:26px;border-radius:0;background-image:url("static/img/cursor/icon_con2.png");top:-13px'
          } else {
            // that.placeholder4 = true
            // that.holder = 'right: 30%'
            that.style.mainLine.related.item2 = 'width:26px;height:26px;background-image:url("static/img/cursor/icon_default.png");top:-13px'
            that.style.mainLine.related.item_c1 = 'width:56px;height:26px;border-radius:0;background-image:url("static/img/cursor/icon_con_default.png");top:-13px'
          }
        } else {
          that.style.mainLine.related.item1 = 'width:26px;height:26px;background-image:url("static/img/cursor/icon_default.png");top:-13px'
          // that.style.mainLine.related.item2 = 'width:26px;height:26px;background-image:url("static/img/cursor/icon_default.png");top:-13px'
          that.style.mainLine.related.item_c = 'width:56px;height:26px;border-radius:0;background-image:url("static/img/cursor/icon_con_default.png");top:-13px'
          that.style.mainLine.related.item_c1 = 'width:56px;height:26px;border-radius:0;background-image:url("static/img/cursor/icon_con_default.png");top:-13px'
        }
        if (that.showData[0].length > 0) {
          that.style.mainLine.showData.item1 = 'width:26px;height:26px;background-image:url("static/img/cursor/icon3.png");top:-13px'
          that.style.mainLine.showData.border1 = 'border-color: #ea6100'
          if (that.isHavingContract(that.showData, 7)) {
            that.style.mainLine.showData.item2 = 'width:26px;height:26px;background-image:url("static/img/cursor/icon3.png");top:-13px'
            that.style.mainLine.showData.border2 = 'border-color: #ea6100'
          } else {
            // that.placeholder2 = true
            // that.holder = 'right: 30%'
            that.style.mainLine.showData.item2 = 'width:26px;height:26px;background-image:url("static/img/cursor/icon_default.png");top:-13px'
          }
        } else {
          that.style.mainLine.showData.item1 = 'width:26px;height:26px;background-image:url("static/img/cursor/icon_default.png");top:-13px'
          that.style.mainLine.showData.item2 = 'width:26px;height:26px;background-image:url("static/img/cursor/icon_default.png");top:-13px'
        }
        if (that.flag) {
          switch (that.flag) {
            case 1:
              ...
              if (!that.dataList[0][0] || !that.dataList[0][0].name) that.placeHolder = []
              break  
            case 2:
              that.flag = 2
              that.dataList = that.related.slice(0, 7)
    ...
    .inline_box {
      ...
      .inline_item {
        ...
        -webkit-border-radius: 100%; 
				...
				z-index: 1;
				display: flex;
				/*background-image: url(img/24_24.png);*/
				background-size: 100%;
