  ...
  <div class="chain_section_wrap">
    <table class="chain_table" cellpadding='0' cellspacing='0'>
      ...
      <tr v-for="(item, index) in tableLine" :key='index' class="chain_table_line">
        ...
        <td>
          <span @click="handleEdit(item, index)">{{!item.isEdit ? '修改' : '保存'}}</span>
    ...
    <div class="input_line" v-if="EOS">
      <div>
        <input type="text" v-model="product_name" maxlength="32">
      </div>
      <div>
        <input type="text" v-model="product_unit" maxlength="8">
      </div>
      <div>
        <input type="text" v-model="product_num" maxlength="16">
      </div>
      <div>
...
export default {
  ...
  methods: {
    ...
    handleSave () {
      ...
      const that = this
      if (that.product_name === '') {
        this.$message.error('请输入货物名称')
      } else {
        if (that.product_unit === '') {
          this.$message.error('请输入正确的获取单位')
        } else {
          if (!(numReg.test(this.product_num))) {
            this.$message.error('请输入正确的货物数量')
          } else {
            let tableLine = that.tableLine
            tableLine.push({
              product_name: that.product_name,
              product_unit: that.product_unit,
