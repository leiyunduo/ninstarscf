  ...
  <div class="c_container">
    <div class="ue_wrap">
      <div class="c_blog_box">
        ...
        <quill-editor ref="myTextEditor" v-model="brief_introduction.content" :config="editorOption"  @ready="onEditorReady($event)">
        </quill-editor>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getBussBlog (bid) {
      var that = this
      if (bid) {
        that.$ajax.get(`/api/home_page/news/` + bid).then(res => {
        ...
        if (that.types === 2) {
          ...
          if (that.id) {
            ...
            that.$ajax.put(`/api/home_page/financial_situation/` + that.id + '/', {dates: that.from.dates, group: that.btnName, body: that.brief_introduction.content, pic_list: []}).then(res => {
              ...
              this.$emit('editState', 'success')
            }).catch(err => {
              that.$message.error(err.response.data.err_msg)
              this.$emit('editState', 'error')
            })
          } else {
            // 发布
  ...
  watch: {
    'brief_introduction.content': function (newer, older) {
      if (newer.trim() !== '') this.btnActive = true
      else this.btnActive = false
    },
    blogTitle: function (newer, older) {
      if (newer.trim() !== '') this.btnActive = true
      else this.btnActive = false
    }
  }
