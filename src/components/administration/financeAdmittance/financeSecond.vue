          ...
          <div class="bas_look">
              <div class="uploaadBtn">
                <el-form v-if='eId !== 1' ref="form" :model="certificates" :rules='rules' label-width="250px" label-position='left' size="small">
                    ...
                    <el-form-item prop='icp_id' label='ICP金融许可：'>
                      <el-input v-model.trim="certificates.icp_id" style="width: 370px;" :maxlength="maxlength"></el-input>
                      <el-upload class="avatar-uploader" v-if="certificates.icp === ''" :action="upDateUrl"
                        :show-file-list="false"
                        :http-request="handleAvatarSuccessD"
                        :before-upload="beforeAvatarUpload">
                        <img v-if="certificates.icp" :src="$root_url() + certificates.icp" class="avatar" alt="ICP金融许可证">
                        <img v-else class="el-icon-plus avatar-uploader-icon" src="../../../../static/img/ICP金融许可证.png" alt="">
                      </el-upload>
                      <div class="img_big" v-if="certificates.icp !== ''">
                        <img :src='$root_url() + certificates.icp' alt='' v-if="certificates.icp !== ''">
                        <div class="hover_img" v-if="certificates.icp !== ''">
                          <span class="ins" @click="bigImg(certificates.icp)"><i class="el-icon-zoom-in"></i></span>
                          <div class="del">
                            <i class="el-icon-upload2">
                              <el-upload class="avatar-uploader" :action="upDateUrl"
                                :show-file-list="false"
                      ...
                      <i v-if="certificates.icp" class="el-icon-close" @click="socialClose($event, 4)"></i>
                    </el-form-item>
                </el-form>
                <el-form v-if='eId === 1' ref="form" :model="certificates" :rules='rules' label-width="250px" label-position='left' size="small">
                  <el-form-item label='法人手持营业执照证件：'>
                  ...
                  <el-form-item label='ICP金融许可：'>
                    <input v-model="certificates.icp_id" :class="{disTables: disTable}" :disabled="disTable">
                    <div class="imgs" v-if="certificates.icp">
                      <div>
                        <img :src="$root_url() + certificates.icp" alt="">
                        <span class="hover_img">
                          <span class="ins" @click="bigImg(certificates.icp)"><i class="el-icon-zoom-in"></i></span>
                        </span>
                      </div>
                    </div><img v-else class="el-icon-plus avatar-uploader-icon" src="../../../../static/img/ICP金融许可证.png" alt="">
                  </el-form-item>
                </el-form>
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        ...
        business_license_id: [
          // { required: true, message: '请填写营业执照证件号', trigger: 'change' },
        ...
        icp_id: [
          { required: true, message: '请填写ICP许可证证件号', trigger: 'change' }
        ]
      },
      tab: '',
      upDateUrl: '',
          ...
          that.$ajax.get(`/api/company/` + that.businessId).then(res => {
            if (res.data.detail_change === null) {
              ...
              that.$ajax.get(`/api/company_detail/` + res.data.detail).then(res => {
                ...
                that.certificates.icp_id = res.data.certificates.icp_id
                that.certificates.icp = res.data.certificates.icp
  ...
  methods: {
    ...
    handleAvatarSuccessD (file) {
      var that = this
      var fileUrl = new FormData()
      that.certificates.icp = file.file
      fileUrl.append('file', that.certificates.icp)
      fileUrl.append('function_module', 'company_info')
      that.$ajax.post('/api/ns_file/', fileUrl).then(res => {
        that.certificates.icp = res.data
      }).catch(err => {
        that.$message.error(err.response.data.err_msg)
      })
    },
      ...
      if (that.certificates.legal_person_business_license === '') {
        ...
        if (this.certificates.business_license_id.length > 18 || this.certificates.business_license_id.length < 15) {
          ...
          if (this.certificates.business_license === '') {
            ...
            if (this.certificates.icp_id === '') {
              this.$message.error('请填写ICP许可证证件号')
            } else {
              if (this.certificates.icp === '') {
                this.$message.error('请上传ICP许可证')
              } else {
                // 社会保险登记证证
                  ...
                  if (valid) {
                    if (that.checked === true) {
                      if (that.is_legal_person === false) {
                        if (that.eId === 2) {
                          if (that.verify === 3) {
                            that.$ajax.put(`/api/company_detail/` + that.putIds + '/', { region_code: that.region_code, ...that.ruleFormFirst, area: that.area, opening_bank_info: that.opening_bank_info, contact_info: that.contact_info, certificates: that.certificates}).then(res => {
                            ...
                            that.$ajax.post('/api/company_detail/', { region_code: that.region_code, change_apply: that.change_apply, ...that.ruleFormFirst, area: that.area, opening_bank_info: that.opening_bank_info, contact_info: that.contact_info, certificates: that.certificates}).then(res => {
                              that.$router.push('/setting?signed=1')
                            }).catch(err => {
                              that.$message.error(err.response.data.err_msg)
                            })
                          ...
                          that.$ajax.post('/api/company_detail/', { region_code: that.region_code, ...that.ruleFormFirst, area: that.area, opening_bank_info: that.opening_bank_info, contact_info: that.contact_info, certificates: that.certificates}).then(res => {
                            that.$router.push('/setting?signed=1')
                          }).catch(err => {
                            that.$message.error(err.response.data.err_msg)
                          })
                        ...
                        if (that.eId === 2) {
                          ...
                          that.$ajax.post('/api/company_detail/', { region_code: that.region_code, ...that.ruleFormFirst, legal_person: that.legal_person_id, area: that.area, opening_bank_info: that.opening_bank_info, contact_info: that.contact_info, certificates: that.certificates}).then(res => {
                            that.$router.push('/setting?signed=1')
                          }).catch(err => {
                            that.$message.error(err.response.data.err_msg)
                          })
