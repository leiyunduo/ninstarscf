<template>
<!-- 资料上传 -->
  <div class="authenticationSecond">
    <div class="right">
              ...
              <div class="uploaadBtn">
                <el-form v-if='eId !== 1' ref="form" :model="certificates" :rules='rules' label-width="250px" label-position='left' size="small">
                    ...
                    <el-form-item class="span_banks" label-width="0">
                      <p>统一社会信用代码（营业执照）：</p>
                      <span class="span_bank">*</span>
                      <!-- <input v-model="certificates.business_license_id" class="disTables" disabled> -->
                      <el-input v-model.trim="certificates.business_license_id" disabled :maxlength="maxlengths" style="width: 370px;"></el-input>
                      <div class="upload_img">
                        <el-upload class="avatar-uploader" v-if="certificates.business_license === ''" :action="upDateUrl"
                            :show-file-list="false"
                            :http-request="handleAvatarSuccess"
                            :before-upload="beforeAvatarUpload">
                            <img v-if="certificates.business_license" :src="$root_url() + certificates.business_license" class="avatar" alt="统一社会信用代码">
                    ...
                    <el-form-item class="avatar-p-list" label='股东信息备案登记表：' v-if="role === 'business'">
                      <el-upload class="avatar-uploader" v-if="certificates.registration_form === ''" :action="upDateUrl"
                          :show-file-list="false"
                          :http-request="registrationForm"
                          :before-upload="beforeAvatarUpload1">
                          <img v-if="certificates.registration_form" :src="$root_url() + certificates.registration_form" class="avatar" alt="股东信息备案登记表">
                          <img v-else class="el-icon-plus avatar-uploader-icon" src="../../../../static/img/股东信息备案登记表.png" alt="">
                      </el-upload>
                      <div class="img_big" v-if="certificates.registration_form !== ''">
                        <img :src='$root_url() + certificates.registration_form' alt='' v-if="certificates.registration_form !== '' && fileData === false">
                        <img src="../../../../static/img/股东信息备案登记表.png" alt='' v-if="certificates.registration_form !== '' && fileData">
                        <div class="hover_img" v-if="certificates.registration_form !== ''">
                          <span class="ins" @click="bigImg(certificates.registration_form)"><i class="el-icon-zoom-in"></i></span>
                          <div class="del">
                            <i class="el-icon-upload2">
                              <el-upload class="avatar-uploader" :action="upDateUrl"
                                :show-file-list="false"
                      ...
                      <p class="pos_file" v-if="certificates.registration_form !== '' && fileData">
                        <a :href="$root_url() + certificates.registration_form" download="test.png" title="点击下载">{{fileDataName}}</a>
                      </p>
                      <i v-if="certificates.registration_form" class="el-icon-close commitment_letter" @click="socialClose($event, 7)"></i>
                      <p class="sapn_poa">下载<a :href="$root_url() + '/nsdoc/ninstar_financial/gdxxba.docx'">股东信息备案登记表</a></p>
                    </el-form-item>
                    <el-form-item class="avatar-p-list" label='承诺函：' v-if="role === 'business'">
                      <el-upload class="avatar-uploader" v-if="certificates.commitment_letter === ''" :action="upDateUrl"
                          :show-file-list="false"
                          :http-request="commitmentLetter"
                          :before-upload="beforeAvatarUpload2">
                          <img v-if="certificates.commitment_letter" :src="$root_url() + certificates.commitment_letter" class="avatar" alt="承诺函">
                          <img v-else class="el-icon-plus avatar-uploader-icon" src="../../../../static/img/承诺函.png" alt="">
                      </el-upload>
                      <div class="img_big" v-if="certificates.commitment_letter !== ''">
                        <img :src='$root_url() + certificates.commitment_letter' alt='' v-if="certificates.commitment_letter !== '' && fileData1 === false">
                        <img src="../../../../static/img/股东信息备案登记表.png" alt='' v-if="certificates.commitment_letter !== '' && fileData1">
                        <div class="hover_img" v-if="certificates.commitment_letter !== ''">
                          <span class="ins" @click="bigImg(certificates.commitment_letter)"><i class="el-icon-zoom-in"></i></span>
                          <div class="del">
                            <i class="el-icon-upload2">
                              <el-upload class="avatar-uploader" :action="upDateUrl"
                                :show-file-list="false"
                      ...
                      <p class="pos_file" v-if="certificates.commitment_letter !== '' && fileData1">
                        <a :href="$root_url() + certificates.commitment_letter" download="test.png" title="点击下载">{{fileDataName1}}</a>
                      </p>
                      <i v-if="certificates.commitment_letter" class="el-icon-close commitment_letter" @click="socialClose($event, 8)"></i>
                      <p class="sapn_poa">下载<a :href="$root_url() + '/nsdoc/ninstar_financial/cnh.docx'">承诺函</a></p>
                    </el-form-item>
                </el-form>
                <el-form v-if='eId === 1' ref="form" :model="certificates" :rules='rules' label-width="250px" label-position='left' size="small">
                  ...
                  <el-form-item label='股东信息备案登记表：' v-if="certificates.registration_form && role === 'business'">
                    <div class="imgs" v-if="certificates.registration_form">
                      <div v-if="fileData === false">
                        <img :src="$root_url() + certificates.registration_form" alt="">
                        <span class="hover_img">
                          <span class="ins" @click="bigImg(certificates.registration_form)"><i class="el-icon-zoom-in"></i></span>
                        </span>
                      </div>
                      <div v-if="fileData">
                        <p class="pos_file" style="padding-left:10px;">
                          <a :href="$root_url() + certificates.registration_form" download="test.png" title="点击下载">{{fileDataName}}</a>
                        </p>
                      </div>
                    </div>
                  </el-form-item>
                  <el-form-item label='承诺函：' v-if="certificates.commitment_letter && role === 'business'">
                    <div class="imgs" v-if="certificates.commitment_letter">
                      <div v-if="fileData1 === false">
                        <img :src="$root_url() + certificates.commitment_letter" alt="">
                        <span class="hover_img">
                          <span class="ins" @click="bigImg(certificates.commitment_letter)"><i class="el-icon-zoom-in"></i></span>
                        </span>
                      </div>
...
<script>
export default {
  name: 'authenticationSecond',
  props: ['ruleFormFirst', 'area', 'nextCompany', 'opening_bank_info', 'contact_info', 'nextData'],
  data () {
    return {
      // 真实性说明
      checked: '',
      ...
      rules: {
        icp_id: [
          { required: true, message: '请填写ICP金融许可', trigger: 'change' }
        ],
        business_license_id: [
        ...
        bank_permit_id: [
          { required: true, message: '请填写银行开户许可证证件号', trigger: 'change' }
        ],
        taxpayer_license_id: [
          { required: true, message: '请填写一般纳税人资格证证件号', trigger: 'change' }
        ]
      },
      tab: '',
      upDateUrl: '',
          ...
          that.$ajax.get(`/api/company/` + that.businessId).then(res => {
            if (res.data.detail_change === null) {
              ...
              that.$ajax.get(`/api/company_detail/` + res.data.detail).then(res => {
                ...
                that.certificates.registration_form = res.data.certificates.registration_form
                that.certificates.commitment_letter = res.data.certificates.commitment_letter
                let str = res.data.certificates.registration_form
                let index = res.data.certificates.registration_form.lastIndexOf("/")
                that.fileDataName = str.substring(index + 1, str.length)
                let flieArr = res.data.certificates.registration_form.split('.')
                let suffix = flieArr[flieArr.length - 1]
                if (suffix === 'docx' || suffix === 'doc' || suffix === 'pdf') {
                  that.fileData = true
                }
                let flieArr1 = res.data.certificates.commitment_letter.split('.')
                let suffix1 = flieArr1[flieArr1.length - 1]
                if (suffix1 === 'docx' || suffix1 === 'doc' || suffix1 === 'pdf') {
                  that.fileData1 = true
                }
                let str1 = res.data.certificates.commitment_letter
                let index1 = res.data.certificates.commitment_letter.lastIndexOf("/")
                that.fileDataName1 = str1.substring(index1 + 1, str1.length) 
              }).catch(err => {
                that.$message.error(err.response.data.err_msg)
              })
            } else {
              that.putIds = res.data.detail_change
              that.$ajax.get(`/api/company_detail/` + res.data.detail_change).then(res => {
                ...
                let str1 = res.data.certificates.commitment_letter
                let index1 = res.data.certificates.commitment_letter.lastIndexOf("/")
                that.fileDataName1 = str1.substring(index + 1, str1.length)
              }).catch(err => {
                that.$message.error(err.response.data.err_msg)
              })
            }
  ...
  methods: {
    ...
    okStep () {
      ...
      that.certificates.registration_form = ''
      that.certificates.commitment_letter = ''
    },
    ...
    registrationForm (file) {
      var that = this
      var fileUrl = new FormData()
      that.certificates.registration_form = file.file
      fileUrl.append('file', that.certificates.registration_form)
      fileUrl.append('function_module', 'company_info')
      that.$ajax.post('/api/ns_file/', fileUrl).then(res => {
        that.certificates.registration_form = res.data
      }).catch(err => {
        that.$message.error(err.response.data.err_msg)
      })
    },
    commitmentLetter (file) {
      var that = this
      var fileUrl = new FormData()
      that.certificates.commitment_letter = file.file
      fileUrl.append('file', that.certificates.commitment_letter)
      fileUrl.append('function_module', 'company_info')
      that.$ajax.post('/api/ns_file/', fileUrl).then(res => {
        that.certificates.commitment_letter = res.data
      }).catch(err => {
        that.$message.error(err.response.data.err_msg)
      })
    },
    ...
    beforeAvatarUpload1 (file) {
      const that = this
      const isJPG = file.type === 'image/jpg' || file.type === 'image/png' || file.type === 'image/jpeg' || file.type === 'application/msword' || file.type === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' || file.type === 'application/pdf'
      const isLt2M = file.size < 5 * 1024 * 1024
      if (!isJPG) {
    ...
    beforeAvatarUpload2 (file) {
      ...
      if (file.type !== 'image/jpg' && file.type !== 'image/png' && file.type !== 'image/jpeg') {
        that.fileDataName1 = file.name
        that.fileData1 = true
      } else {
        that.fileData1 = false
      }
      return isJPG && isLt2M
    },
    // 下一步
            ...
            if (valid) {
              if (that.checked === true) {
                if (that.is_legal_person === false) {
                  if (that.eId === 2) {
                    if (that.verify === 3) {
                      that.$ajax.put(`/api/company_detail/` + that.putIds + '/', { region_code: that.region_code,name: that.ruleFormFirst.name,industry_type: that.ruleFormFirst.industry_type, company_nature: that.ruleFormFirst.company_nature, area: that.area, opening_bank_info: that.opening_bank_info, contact_info: that.contact_info, certificates: that.certificates}).then(res => {
                      ...
                      that.$ajax.post('/api/company_detail/', { region_code: that.region_code, name: that.ruleFormFirst.name,change_apply: that.change_apply, industry_type: that.ruleFormFirst.industry_type, company_nature: that.ruleFormFirst.company_nature, area: that.area, opening_bank_info: that.opening_bank_info, contact_info: that.contact_info, certificates: that.certificates}).then(res => {
                        that.$router.push('/setting?signed=1')
                      }).catch(err => {
                        that.$message.error(err.response.data.err_msg)
                      })
                    ...
                    that.$ajax.post('/api/company_detail/', { region_code: that.region_code, name: that.ruleFormFirst.name,industry_type: that.ruleFormFirst.industry_type, company_nature: that.ruleFormFirst.company_nature, area: that.area, opening_bank_info: that.opening_bank_info, contact_info: that.contact_info, certificates: that.certificates}).then(res => {
                      that.$router.push('/setting?signed=1')
                    }).catch(err => {
                      that.$message.error(err.response.data.err_msg)
                    })
                  }
                } else if (that.is_legal_person === true) {
                  if (that.eId === 2) {
                    if (that.verify === 3) {
                      that.$ajax.put(`/api/company_detail/` + that.putIds + '/', { region_code: that.region_code, name: that.ruleFormFirst.name,industry_type: that.ruleFormFirst.industry_type, company_nature: that.ruleFormFirst.company_nature, area: that.area, opening_bank_info: that.opening_bank_info, contact_info: that.contact_info, certificates: that.certificates}).then(res => {
                        that.$router.push('/setting?signed=1')
                      ...
                      that.$ajax.post('/api/company_detail/', { region_code: that.region_code, name: that.ruleFormFirst.name,change_apply: that.change_apply, company_nature: that.ruleFormFirst.company_nature, industry_type: that.ruleFormFirst.industry_type, area: that.area, opening_bank_info: that.opening_bank_info, contact_info: that.contact_info, certificates: that.certificates}).then(res => {
                        that.$router.push('/setting?signed=1')
                      }).catch(err => {
                        that.$message.error(err.response.data.err_msg)
                      })
                    ...
                    that.$ajax.post('/api/company_detail/', { region_code: that.region_code, name: that.ruleFormFirst.name, company_nature: that.ruleFormFirst.company_nature,legal_person: that.legal_person_id, industry_type: that.ruleFormFirst.industry_type, area: that.area, opening_bank_info: that.opening_bank_info, contact_info: that.contact_info, certificates: that.certificates}).then(res => {
                      that.$router.push('/setting?signed=1')
                    }).catch(err => {
                      that.$message.error(err.response.data.err_msg)
                    })
