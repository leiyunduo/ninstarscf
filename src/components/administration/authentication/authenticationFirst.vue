<template>
  <div class="authenticationFirst">
    <div class="info-enter">
      ...
      <authentication-second v-if='nextData === 1' @nextDataShow='nextDataShow' :putId='putId' :certificate='certificate' @dataLast='dataLast' :contact_info='contact_info' :nextCompany='nextCompany' :area='area' :opening_bank_info='opening_bank_info' :nextData='nextData' :ruleFormFirst='ruleForm' />
      <div class="right" v-if='nextData === 0'>
        <!-- 基础信息 -->
        <div class="right_top">
          <div class="h3">
            <h3>企业基础信息<span>修改你的基本资料，将显示在你的个人名片中，并可以让更多的朋友了解和认识你！</span></h3>
          </div>
        </div>
        <div class="basicsList">
          <el-form :model="ruleForm" :rules="rules" ref="ruleFormO" class="demo-ruleForm" label-position='left' size="small">
            <el-form-item label="企业名称:" prop="name" class="basics_span_class" label-width="100px">
              <span class="basics_span" style="display:block">企业名称需要以真实的企业/机构/团队的名称注册，与统一社会信用代码执照保持一致</span>
            ...
            <el-form-item label="企业类型：" prop="company_nature" label-width="189px" v-show="eId !== 1 && role === 'business'" style="padding-left:11px;">
              <el-select v-model.trim="ruleForm.company_nature" placeholder="请选择" style="width: 370px;">
                <el-option label="企业" value="1"></el-option>
                <el-option label="个体工商户" value="2"></el-option>
                <el-option label="政府机关/事业单位" value="3"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="企业类型：" prop="company_nature" style="padding-left:11px;" label-width="189px" v-show="eId === 1 && role === 'business' && ruleForm.company_nature">
              <el-input v-model="status[ruleForm.company_nature]" class="inp_iner" style="width: 370px;" :class="{disTables: disTable}" :disabled="disTable"></el-input>
            </el-form-item>
            <el-form-item label="服务商类型：" prop="industry_type" label-width="189px" v-show="eId !== 1 && role === 'serviceprovider'" style="padding-left:11px;">
              <el-select v-model.trim="ruleForm.industry_type" placeholder="请选择" style="width: 370px;">
                <el-option label="物流" value="1"></el-option>
                <el-option label="仓储" value="2"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="服务商类型：" prop="industry_type" style="padding-left:11px;" label-width="189px" v-show="eId === 1 && role === 'serviceprovider' && ruleForm.industry_type">
              <el-input v-model="serviceTypeStatus[ruleForm.industry_type]" class="inp_iner" style="width: 370px;" :class="{disTables: disTable}" :disabled="disTable"></el-input>
            </el-form-item>
            <el-form-item label="行业：" prop="industry_type" v-show="eId !== 1 && role === 'business'" style="padding-left:11px;" label-width="189px">
              <el-select v-model.trim="ruleForm.industry_type" placeholder="请选择" style="width: 370px;">
                ...
                <el-option label="冶金矿产" value="冶金矿产"></el-option>
                ...
                <el-option label="机械机电" value="机械机电"></el-option>
                <el-option label="轻工食品" value="轻工食品"></el-option>
                <el-option label="服装纺织" value="服装纺织"></el-option>
                <el-option label="专业服务" value="专业服务"></el-option>
                <el-option label="安全防护" value="安全防护"></el-option>
                <el-option label="环保绿化" value="环保绿化"></el-option>
                <el-option label="旅游休闲" value="旅游休闲"></el-option>
                <el-option label="办公文教" value="办公文教"></el-option>
                <el-option label="电子电工" value="电子电工"></el-option>
                <el-option label="玩具礼品" value="玩具礼品"></el-option>
            ...
            <el-form-item label="行业：" prop="industry_type" v-show="eId === 1 && role === 'business' && ruleForm.industry_type" style="padding-left:11px;" label-width="189px">
              <el-input v-model="ruleForm.industry_type" class="inp_iner" style="width: 370px;" :class="{disTables: disTable}" :disabled="disTable"></el-input>
            </el-form-item>
        ...
        <div class="basicsList">
          <el-form :model="contact_info" v-if='eId !== 1' :rules="rules" ref="ruleForm" label-position='left' size="small">
            ...
            <el-form-item label="公司邮箱:" style="padding-left:11px;" prop="email" label-width="189px">
          ...
          <el-form :model="contact_info" v-if='eId === 1' :rules="rules" ref="ruleForm" label-position='left' size="small">
            ...
            <el-form-item label="公司邮箱:" v-if='contact_info.email' style="padding-left:11px;" prop="email" label-width="189px">
              <el-input v-model.trim="contact_info.email" style="width: 370px;" :class="{disTable: disTable}" :disabled="disTable" :maxlength="maxlength"></el-input>
            </el-form-item>
...
import authenticationSecond from './authenticationSecond'
export default {
  data () {
    return {
      // 真实性说明
  ...
  mounted () {
    ...
    that.$ajax.get('/api/company/status/').then(res => {
      ...
      that.changes = res.data.change_apply
      // 查看详情
      if (that.eId) {
        that.eId = Number(that.$route.query.eId)
        if (that.eId === 2) {
          ...
          that.$ajax.get(`/api/company/` + that.businessId).then(res => {
            if (res.data.detail_change === null) {
              ...
              that.$ajax.get(`/api/company_detail/` + res.data.detail).then(res => {
                ...
                that.ruleForm.company_nature = res.data.company_nature
