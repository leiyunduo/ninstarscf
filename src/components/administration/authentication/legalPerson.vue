          ...
          <div class='basicsTitle'>
            <el-form :model='ruleForm' :rules='rules' ref='ruleForm' class='ruleForm' label-width="150px" label-position='left' size="small">
              ...
              <el-form-item label='身份证号：' prop='id_number'>
                <el-input v-model.trim='ruleForm.id_number' @blur="idNumber(ruleForm.id_number)" style="width: 370px;" :class="{disTables: disTable}" :disabled="disTable" :maxlength="maxlength"></el-input>
              </el-form-item>
              <!-- <el-form-item label='手机号：' prop='phone' v-if='ruleForm.person_info.phone'>
                <el-input v-model.trim='ruleForm.person_info.phone' style="width: 370px;" :class="{disTables: disTable}" :disabled="disTable" :maxlength="maxlengths - 1"></el-input>
              </el-form-item> -->
              <!-- <el-form-item label='获取验证码：' prop='v_code' class='verificationCode' v-if='eId !== 1'>
                <el-input v-model.trim='ruleForm.v_code' style="width: 220px;" :class="{disTables: disTable}" :disabled="disTable" class='verification'></el-input>
                <div class="button_class_list" size='mini' @click='postVerific' :disabled='isDisabled' v-if='eId !== 1'>{{buttonName}}</div>
              </el-form-item> -->
                  ...
                  <li v-for='(item, index) in imgList' :key='index'>
                    <div class='uploaadBtn uploaadBtns'>
                      ...
                      <div class="img_big" v-if="item.img !== ''">
                        ...
                        <div class="hover_img" v-if="item.img !== ''">
                          <span class="ins" @click="bigImg(item.img)"><i class="el-icon-zoom-in"></i></span>
                          <span class="del">
                            <i class="el-icon-upload2">
                              <input type='file' :id="item.title" @change='uploaadFile($event, index)'>
                            </i>
                          </span>
                        </div>
                      </div>
              ...
              <!-- <el-form-item label='邮箱:' prop='email'>
                <el-input v-model.trim='ruleForm.person_info.email' :class="{disTables: disTable}" :disabled="disTable" :maxlength="maxlength" style="width: 370px;"></el-input>
              </el-form-item>
              <el-form-item label='家庭住址:' prop='home_address'>
                <el-input v-model.trim='ruleForm.person_info.home_address' :class="{disTables: disTable}" :disabled="disTable" :maxlength="maxlength * 2" style="width: 370px;"></el-input>
              </el-form-item>
              <el-form-item label='教育程度：' prop='education' v-if='eId !== 1'>
                <el-select v-model.trim='ruleForm.person_info.education' placeholder='请选择' style="width: 220px;">
              ...
              <el-form-item label='教育程度：' prop='education' v-if='eId === 1'>
                <el-input v-model="educationStatus[ruleForm.person_info.education]" class="inp_iner" :class="{disTables: disTable}" style="width: 370px;" :disabled="disTable"></el-input>
              </el-form-item>
              <el-form-item label='年龄状况：' prop='age' v-if='eId !== 1'>
                <el-select v-model.trim='ruleForm.person_info.age' placeholder='请选择' style="width: 220px;">
                  <el-option label='18岁—40岁' value='0'></el-option>
                  <el-option label='41—65岁' value='1'></el-option>
                  <el-option label='66岁以后' value='2'></el-option>
                </el-select>
              </el-form-item>
              <el-form-item label='年龄状况：' prop='age' v-if='eId === 1'>
                <el-input v-model="ageStatus[ruleForm.person_info.age]" class="inp_iner" :class="{disTables: disTable}" :disabled="disTable" style="width: 370px;"></el-input>
              </el-form-item>
              <el-form-item label='从事行业年限:' prop='years_pro'>
                <el-input v-model.trim='ruleForm.person_info.years_pro' :class="{disTables: disTable}" :disabled="disTable" style="width: 370px;"></el-input>
              </el-form-item> -->
            </el-form>
        ...
        that.$ajax.get(`/api/company/` + that.businessId).then(res => {
          if (that.changes === false || that.verify === 1) {
            that.$ajax.get(`/api/company_detail/` + res.data.detail).then(res => {
              ...
              if (res.data.legal_person_info.person_info) {
                that.ruleForm.person_info = res.data.legal_person_info.person_info
              }
