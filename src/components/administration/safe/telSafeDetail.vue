<template>
  <div class="c_container">
    <div class="c_right_content">
      <div v-if="isBind === '1' && !showNew" class="c_telbox">
        <p class="c_gray">保障账户中重要信息，在设置、修改登录密码等都需要手机短信验证及接收异常消息。绑定后可以使用手机号登入平台。</p>
        ...
        <div class="c_form">
          <el-form ref="telForm" label-position="left" :model="telForm" :rules="rules">
            <el-form-item>
              <p>原手机号码：</p>
              <el-input v-model="telForm.oldTelNum" style="width:240px;" disabled size="mini"></el-input>
            </el-form-item>
      ...
      <div v-if="isBind === '0' && !showNew" class="c_telbox">
      ...
      <div v-if="showNew" class="c_telbox">
        <p class="c_gray">保障账户中重要信息，在设置、修改登录密码等都需要手机短信验证及接受异常消息。绑定后可以使用手机号登入平台。</p>
        <p class="c_tip"><span class="c_msg_box"><span class="c_tip_msg">请验证您的手机号码，点击“获取验证码”完成验证</span></span></p>
        <div class="c_form">
          <el-form ref="newTelForm" label-position="left" :model="newTelForm" :rules="rules">
            <el-form-item prop="newTelNum">
              <p>请输入手机号码：</p>
              <el-input v-model="newTelForm.newTelNum" style="width:240px;" size="mini"></el-input>
            </el-form-item>
            <el-form-item prop="code">
              <p><span>*</span>请填写短信验证码：</p>
              <el-input v-model="newTelForm.code" style="width:130px;" size="mini" class="c_code_input"></el-input>
              <a class="c_get_code c_btns" :class="buttonName === '获取验证码' ? 'blue_class' : 'gay_class'" @click="getCode('newTelForm')">{{ buttonName }}</a>
            </el-form-item>
            <el-form-item>
              <a class="c_primary_btn" type="primary" style="display:inline-block;margin-right:20px;" @click="onUpper('emailForm')">上一步</a>
...
export default {
  ...
  methods: {
    ...
    getCode (formName) {
      if (this.newTelForm.newTelNum === this.telForm.oldTelNum) {
      ...
      if (formName === 'newTelForm') {
        let val = this.newTelForm.newTelNum
        let data = {}
        data.phone = this.newTelForm.newTelNum
        data.operation_type = 'login_name'
        that.$ajax.get('/api/account_exists/?phone=' + this.newTelForm.newTelNum).then(res => {
    ...
    onSubmit () {
      const that = this
      // console.log(this.newTelForm.newTelNum)
      if (this.newTelForm.newTelNum === this.telForm.oldTelNum) {
        this.$message.error('请输入不同的手机号')
        return
      }
      let reg = /^[1][3,4,5,7,8][0-9]{9}$/
      if (!(reg.test(this.newTelForm.newTelNum)) || this.newTelForm.code.trim() === '') {
        this.$message.error('请输入正确的信息')
        return
      }
      var data = {}
      data.phone = this.newTelForm.newTelNum
      data.user_key = that.newTelForm.code
