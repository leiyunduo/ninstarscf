    ...
    <div class="creditor-debt clearfix">
      ...
      <div class="inner_box">
        ...
        <div class="btn-box fail">
          <button class="fund" v-if="!isJump" @click="toCash">贴现</button>
          <button class="cancel" v-if="!isJump" @click="changePay">转移支付</button>
          <button class="cancel" v-if="isJump" @click="jumpBack">返回</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  mounted () {
    ...
    that.$ajax.get('/api/claims_debt/claims/management/?id=' + that.idTab + '&token=' + that.$token()).then((res) => {
      if (res.data.code === '1') {
