    ...
    sureOperation () {
      ...
      this.$ajax.post('/api/identity_auth/', {token: this.$token(), ...data}).then(res => {
        if (res.data.code === '1') {
          that.buttonName = '获取验证码'
          window.clearInterval(that.interval)
          this.$ajax.delete('/api/claims_debt/debt_management/?debts_management_id='+this.$route.query.id+'&token='+this.$token()).then(res => {
            // console.log(res)
    ...
    goback () {
      this.$router.push({
        ...
        query: {
          ...
          delete: this.$route.query.delete
        }
      })
    }
  },
