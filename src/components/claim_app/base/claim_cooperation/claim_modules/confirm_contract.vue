...
export default {
  ...
  methods: {
    ...
    changeTab (item, index) {
      ...
      this.setIndex = index
      this.$ajax.get(`/api/chain/contract_show/?contract_id=${item.id}&partner=1`).then(res => {
        if(res.data.has) {
  ...
  mounted () {
    ...
    this.$ajax.get(`/api/chain/contract_show?contract_id=${this.id}&partner=1`).then(res => {
      if(res.data.has) {
        this.contract = res.data
